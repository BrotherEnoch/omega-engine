...
mod tests {
    ...
    fn always_at_most_500_gwei() {
        for bonus in [0.001, 0.1, 1.0, 10.0, 100.0] {
            for hf in [1.0, 1.0005, 1.003, 1.008, 1.05] {
                ...
                assert!(
                    cap <= MAX_PRIORITY_FEE_GWEI,
                    "cap={cap} bonus={bonus} hf={hf}"
                );
            }
        }
    }
    ...
    fn urgency_tiers() {
        assert_eq!(
            UrgencyTier::from_health_factor(1.0000),
            UrgencyTier::Critical
        );
        assert_eq!(
            UrgencyTier::from_health_factor(1.0009),
            UrgencyTier::Critical
        );
        ...
        assert_eq!(
            UrgencyTier::from_health_factor(1.005),
            UrgencyTier::Moderate
        );
        assert_eq!(
            UrgencyTier::from_health_factor(1.009),
            UrgencyTier::Moderate
        );
