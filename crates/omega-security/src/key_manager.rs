...
//       if k == execution_key → true
//       if pending_key != 0 && k == pending_key && block <= window_end → true
//
...
//
// State machine:
//   Active     — one live execution key, no rotation in progress.
//   Rotating   — pending_key set; both keys accepted for rotation_window_blocks.
//   Completing — rotation window closing; next block_tick() completes it.
//
// Thread-safety:
...
/// Observable rotation state (no key bytes — safe to serialize/log).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum KeyRotationState {
    /// Single active key; no rotation pending.
    Active {
        /// Ethereum address (hex) of the active key — safe to log.
        address: String,
    },
...
impl KeyManager {
    ...
    /// Lock-contention: RwLock read — multiple signers can read concurrently.
    #[inline]
        ...
        if needs_complete {
                ...
                (s.pending_key.take(), s.pending_addr.take(), s.window_end.take())
            {
                ...
                tracing::warn!(
                    ...
                    "key rotation completed — old key decommissioned"
                );
            }
        }
    }
