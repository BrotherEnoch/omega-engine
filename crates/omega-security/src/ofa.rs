...
//
// Spec references:
//   Section 8  — "Security + OFA compliance + versioned rules"
//   config/ofa_rules.toml — rule schema (version, effective_date, [[rules]])
//   omega-compliance crate — this module provides the rule data; compliance
//                            orchestration sits in omega-compliance.
//
// OFA rule types (from spec ofa_rules.toml):
//   RequireConsentSig    — user must sign a consent message before their tx
//                          is backrun.  Blueprint must carry the sig.
//   EnforceUserSlippage  — omega's extractable value from the user tx must not
//                          exceed max_excess_bps basis points above their stated
//                          slippage tolerance.
//   EnforceBundleOrder   — user tx must appear BEFORE omega tx in the bundle.
//                          Prevents front-running in the same bundle.
//   PrivateRelayOnly     — bundles must ONLY be sent to the allowed private
...
//   4. The registry hot-swaps the rule set (ArcSwap — zero-downtime).
...
impl OfaRuleRegistry {
    ...
    /// require OFA compliance — callers should gate on `blueprint.ofa_compliant`.
    pub fn check(&self, input: &OfaComplianceInput) -> OfaComplianceResult {
        ...
        let rule_set = match guard.as_ref().as_ref() {
            Some(rs) => rs,
            None => {
                // Rules not loaded — this is a configuration error, not a violation.
                // Log and pass through so the system doesn't halt on startup ordering.
                tracing::warn!(
                    blueprint = %input.blueprint_hash,
                    "OFA rules not loaded — compliance check skipped"
                );
...
// ─── Helper: compliant input builder ─────────────────────────────────────────
...
mod ofa_tests {
    ...
    fn unloaded_registry_passes_with_warning() {
        // Rules not loaded → check passes (logged as warning, not error)
        let reg = OfaRuleRegistry::new();
        assert!(reg.check(&compliant_input()).is_compliant());
    }
    ...
    // ── Fast-fail: first violation returned ──────────────────────────────────
