...
//
// Pattern detector — rolling-window `LOST_RACE_SAME_FEE` fingerprint guard
// (spec §14).
//
// ## Purpose
//
...
// ## Thread safety
//
//   `PatternDetector` is NOT `Send + Sync` — it is owned exclusively by
...
/// Configuration for the fingerprinting pattern detector (§14).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PatternDetectorConfig {
    /// Number of loss events in the rolling window.
    ///
    /// Default 200 — provides enough statistical signal to distinguish
...
/// Rolling-window `LOST_RACE_SAME_FEE` fingerprint detector (§14).
///
/// Not `Send + Sync` — owned by the rotation manager task only.
...
impl PatternDetector {
    ...
    /// to be crossed — the caller should trigger an address rotation.
    ...
    /// Reset the window — called after a rotation so the detector starts
...
mod tests {
    ...
    fn trigger_requires_min_events() {
        let mut d = detector(); // min_events = 5
                                // 3 same-fee in 4 events = 75%, but < min_events → no trigger
        for _ in 0..4 {
            d.record(LossCode::LostRaceSameFee);
        }
        assert!(
    ...
    fn window_evicts_oldest() {
        let mut d = detector(); // window_size = 10
                                // Fill with same-fee → rate = 100%
        for _ in 0..10 {
            d.record(LossCode::LostRaceSameFee);
        }
        assert!(d.same_fee_rate() > 0.99);

        // Push 10 non-same-fee events — should evict all same-fee entries
        for _ in 0..10 {
            d.record(LossCode::LostGasLow);
        }
    ...
    fn record_returns_trigger_status() {
        ...
        // 5th event, all different — no trigger
        let triggered = d.record(LossCode::LostRaceSameFee);
        // 1/5 = 20% — threshold is STRICTLY greater than, so 20% == 20% → no trigger
        assert!(!triggered);
