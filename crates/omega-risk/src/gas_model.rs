...
pub fn dynamic_min_profit(
    ...
    let l1_cost =
        (l1_data_gas as f64 * current_l1_gas_price as f64 * l1_adaptive_buf).ceil() as u64;
    let ext_cost =
        (EXTRACTION_GAS as f64 * current_l2_base_fee as f64 * L2_EXEC_BUFFER).ceil() as u64;
...
mod gas_model_tests {
    ...
    fn high_volatility_clamps_at_max_buffer() {
        ...
        assert!(
            (1.30..=L1_BUFFER_MAX).contains(&buf),
            "buffer {} out of [1.30, 2.00]",
            buf
        );
        assert!(
            (buf - L1_BUFFER_MAX).abs() < 0.01,
            "expected ~2.00, got {}",
            buf
        );
    }
