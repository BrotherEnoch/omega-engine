...
//
//   2. Gas model ↔ checks integration: l1_adaptive_buffer output fed into
//      dynamic_min_profit and then into a blueprint's dynamic_min_profit_wei.
//
//   3. Competition score ↔ checks: competition_probability output fed into
//      CheckContext.competition_probability to confirm check 11 gate.
//
//   4. Circuit breaker ↔ EV recovery: record() → state transition → l2/l3 resume.
//
//   5. Flash-crash guard ↔ checks context: FlashCrashResponse fields map onto
...
//
//   7. L1GasEma rolling window → adaptive buffer → min profit progression.
//
//   8. Metric counters increment on pass/fail (smoke-test only — no actual Prometheus
...
fn base_ctx() -> CheckContext {
    CheckContext {
        ...
        oracle: OracleSnapshot {
            ...
            pyth_price:      3001.5,   // 0.05 % divergence — within 0.4 %
            twap_price:      2998.0,
            chainlink_age_s: 5,
            pyth_age_s:      8,
            twap_age_s:      30,
...
// ─── 2. Gas model ↔ checks integration ───────────────────────────────────────
...
fn competition_probability_gates_check_11() {
    ...
    // High competition probability exceeds threshold → fail
    ...
    // Same probability exactly at threshold — must fail (strict >)
    ...
    // Just below threshold — must pass
...
fn priority_fee_always_within_bounds() {
    // Various inputs — fee must always be in [2, 500].
    let cases = [
        (0.0, 1.10, 0.90),
        (0.001, 1.0001, 0.05),
        (50.0, 1.0001, 0.05),
...
fn circuit_breaker_full_lifecycle() {
    ...
    for _ in 0..EV_WINDOW_BLOCKS {
        reg.record("LA", 0.3, 1.0); // EV = 0.3 < 0.50 → Halted
    }
    ...
    assert!(reg.is_operational("LA"));
    // Window cleared → EV ratio = 1.0
    assert!((reg.ev_ratio("LA") - 1.0).abs() < 1e-9);

    // Phase 4: degrade to AutoPaused
    for _ in 0..EV_WINDOW_BLOCKS {
        reg.record("LA", 0.65, 1.0); // EV = 0.65 → AutoPaused
    }
...
fn investigate_state_is_operational_but_not_healthy() {
    let reg = CircuitBreakerRegistry::new();
    for _ in 0..EV_WINDOW_BLOCKS {
        reg.record("MSA", 0.78, 1.0); // 0.70 ≤ 0.78 < 0.85 → Investigate
    }
...
fn multiple_strategies_isolated() {
    ...
    // LA was never recorded — defaults to Healthy
    assert_eq!(reg.state("LA"),  CircuitState::Healthy);
}

// ─── 5. Flash-crash guard ↔ context ──────────────────────────────────────────
...
fn flash_crash_tightens_oracle_agreement_in_context() {
    ...
    match guard.evaluate(1.05) {
        crate::flash_crash::FlashCrashResponse::Graduated { oracle_agreement_pct, .. } => {
            // Tightened from 0.4 % to 0.1 % — confirm the field value.
...
fn flash_crash_imminent_liquidation_bypasses_size_reduction() {
    ...
    let resp = guard.evaluate(1.0005); // HF < 1.001 → imminent
    assert!(
...
fn ema_window_fills_and_computes_correct_buffer() {
    ...
    // Constant prices → CV = 0 → buffer stays at minimum.
    assert!((ema.current_buffer() - L1_BUFFER_MIN).abs() < 1e-9);
...
// ─── 8. Gas spike boundary ───────────────────────────────────────────────────

#[test]
fn gas_spike_threshold_boundary_conditions() {
    let bp = base_bp(); // l1_data_fee_at_creation = 40

    // Exactly 30 % increase: 40 × 1.30 = 52 — NOT strictly greater, should pass.
    let mut ctx = base_ctx();
    ctx.current_l1_gas_price_gwei = 52;
    assert_eq!(run_all_checks(&bp, &ctx), CheckResult::Pass,
        "30% delta exactly equals threshold — should pass (not strictly greater)");

    // 31 % increase: 40 × 1.31 = 52.4 → 53 gwei — should fail.
...
fn one_fresh_oracle_sufficient() {
    ...
    ctx.oracle.twap_age_s      = 10;   // fresh — one is enough
    assert_eq!(run_all_checks(&bp, &ctx), CheckResult::Pass);
}
