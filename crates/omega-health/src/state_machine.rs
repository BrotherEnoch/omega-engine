...
//
// 14-layer Health FSM — concrete implementation of the LayerHealth trait.
//
// TLA+ spec: formal/health_fsm.tla
//
// Spec §3 — three-state FSM per layer:
//
//   Healthy  → Degraded  : non-fatal anomaly detected
//   Healthy  → Halted    : catastrophic fault (skip Degraded when needed)
//   Degraded → Halted    : threshold crossed or cascading fault
//   Degraded → Healthy   : transient fault cleared (automatic recovery)
//   Halted   → Healthy   : governance clearance only (§5)
//   Halted   → Degraded  : partial recovery — not in TLA+ spec; blocked
//
// Blocked transitions:
//   Halted → Degraded    : disallowed — recovery is always to Healthy
//   (same state → same)  : no-op with WARN log to detect logic errors
//
...
//   SystemHealth orchestrator can react (halt propagation, §3).
//   The tracing event is the secondary record; the log entry is primary.
//
// Locking:
//   `RwLock<HealthState>` — many readers (is_operational polls), one writer
...
//   `persistence` or `propagation` — those are called after the write
//   lock is released to avoid lock inversion.

use std::sync::{Arc, RwLock};

use chrono::Utc;
use omega_core::{HealthState, LayerHealth, LayerId};
...
pub enum TransitionError {
    /// `Halted → Degraded` is not a valid recovery path.
    /// Recovery from Halted must go directly to Healthy (§3).
    #[error(
        "Invalid transition {from:?} → {to:?} for layer {layer:?}: Halted may only recover to Healthy"
    )]
    ...
    /// error — set_state should only be called when a state change is
    /// required.
    #[error("No-op transition {state:?} → {state:?} for layer {layer:?}")]
    NoOp { layer: LayerId, state: HealthState },
}
...
impl LayerHealthImpl {
    ...
    ///
    /// `log` and `tx` are optional — pass `None` in tests or when
    /// those subsystems are not yet initialised.
    pub fn new(id: LayerId, log: Option<HealthLog>, tx: Option<TransitionSender>) -> Arc<Self> {
        Arc::new(Self {
            state: RwLock::new(HealthState::Healthy),
            layer_id: id,
            log: log.map(std::sync::Mutex::new),
            tx,
        })
    }
    ...
    pub fn try_transition(
        ...
        if old_state == new_state {
            ...
            // Not an error — log at WARN and return Ok so callers don't
        ...
        match new_state {
            HealthState::Healthy | HealthState::Recovering => tracing::info!(
                layer  = %self.layer_id,
                from   = %old_state,
                to     = %new_state,
                reason = reason,
            ...
            HealthState::Degraded | HealthState::Unknown => tracing::warn!(
                layer  = %self.layer_id,
                from   = %old_state,
                to     = %new_state,
                reason = reason,
                ...
                Ok(mut log) => {
                    if let Err(e) = log.append(&entry) {
                        tracing::error!(
                            layer = %self.layer_id,
                            error = %e,
                            "Failed to persist health log entry — transition already applied",
                        );
                    }
                }
                Err(e) => {
        ...
        if let Err(e) = self.try_transition(new_state, reason) {
            match &e {
                ...
                TransitionError::HaltedToDegragedBlocked { .. } => {
                    tracing::error!(
                        error = %e,
                        "Blocked illegal health FSM transition — layer remains Halted",
                    );
                }
            }
        }
...
mod tests {
    ...
    fn healthy_to_halted_skipping_degraded() {
        // Spec §3 allows direct Healthy → Halted on catastrophic faults.
        let l = layer();
    ...
    fn halted_to_degraded_is_blocked() {
        let l = layer();
        l.set_state(HealthState::Halted, "fault");
        // Must be blocked — state must remain Halted
        let result = l.try_transition(HealthState::Degraded, "invalid recovery");
        assert!(matches!(
            result,
            Err(TransitionError::HaltedToDegragedBlocked { .. })
        ));
        assert_eq!(
            l.state(),
            HealthState::Halted,
