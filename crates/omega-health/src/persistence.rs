...
//
// HealthLog — append-only, newline-delimited JSON log of all health FSM
// transitions.
//
// Spec §16:
//   Health state transitions are always-sampled events (100% sampling
//   rate).  The log is the primary audit record — the tracing event in
...
//   responsibility of the operator (logrotate / systemd) — the engine
...
//   throughput — transitions are infrequent (<1/s in normal operation)
//   so the buffering is conservative.  If stronger durability is needed
//   (survives kernel crash) callers should wrap with `sync_all` — not
...
pub struct HealthLogEntry {
    ...
    /// Layer that transitioned — canonical snake_case label (§2).
...
impl HealthLogEntry {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.from_state == self.to_state {
            return Err(format!(
                "no-op entry: {} → {} for layer {}",
                self.from_state, self.to_state, self.layer_id,
            ));
        }
        // Blocked transition: HALTED → DEGRADED
        if self.from_state == "HALTED" && self.to_state == "DEGRADED" {
            return Err(format!(
                "blocked transition HALTED → DEGRADED for layer {}; \
...
///
/// `HealthLog` is not `Clone` — there must be exactly one writer per
...
impl HealthLog {
    ...
    /// caller (state_machine.rs) logs and continues — persistence
...
mod tests {
    ...
    fn entry_validation_accepts_valid_transitions() {
        ...
        for (from, to) in valid {
            ...
            assert!(e.validate().is_ok(), "expected valid: {from} → {to}");
        }
    }
}
