...
//
// Health monitors — background tasks that observe system metrics and
...
//
// Spec references:
//   §3    — Health FSM; monitors drive Healthy ↔ Degraded ↔ Halted
//   §11.1 — LA tier monitor: hot/warm/cold/archived position counts
//   §11.4 — Reorg guard integration
//   §16   — Observability: monitor events are always-sampled
//
// ## Monitors implemented here
//
//   OracleLivenessMonitor  — watches oracle heartbeat timestamps.
//     Degraded if any feed is silent for > oracle_stale_threshold_ms.
//     Halted if ALL primary feeds are simultaneously silent.
//
//   GasSpikeMonitor        — watches the FeeSnapshot channel.
//     Degraded when base_fee_gwei exceeds the configured spike threshold.
//     Recovered when base_fee drops back below threshold.
//     Used by the Gas War Engine to trigger MissGasSpike drops (§7).
//
//   HaltPollLoop           — the 10ms polling loop that checks HaltFlag
//     and forwards it to the relay/scoring shutdown channels.
//     SLA: halt visible to all loops within 200ms (§3).
...
///
///   - Any feed silent > `stale_threshold` → Degraded
///   - All primary feeds simultaneously silent → Halted
///
/// Spec §3: ExternalData Halted cascades to Eil, Strategy, HotPath.
    ...
    fn evaluate(&self) {
        ...
        if current != target {
            let reason = match target {
                ...
                HealthState::Recovering => "oracle feeds recovering",
                HealthState::Unknown => "oracle feed state unknown",
            };
        ...
        match (current, is_spiking) {
            (HealthState::Healthy, true) => {
                tracing::warn!(
                    ...
                    "Gas spike detected — transitioning to DEGRADED",
                );
            ...
            (HealthState::Degraded, false) => {
                tracing::info!(
                    base_fee_gwei = fee_gwei,
                    "Gas spike resolved — recovering to HEALTHY",
                );
                self.layer.set_state(
                    HealthState::Healthy,
                    &format!(
                        "gas spike resolved: {fee_gwei} gwei ≤ threshold {}",
                        self.spike_threshold_gwei
                    ),
                );
            }
            // Halted layers are not recovered by the gas monitor —
            // recovery requires governance clearance (§5).
            _ => {}
        }
    }
}
...
/// 10ms halt polling loop — bridges HaltFlag to a `watch` channel.
///
/// SLA: halt visible to all loops within 200ms (§3).
...
impl HaltPollLoop {
    ...
    pub async fn run(self, mut shutdown: tokio::sync::watch::Receiver<bool>) {
        ...
        // Seed from the watch channel's published state, not the current flag.
        // This preserves edge transitions that occur after `new()` but before
        // the spawned poll loop gets scheduled.
        let mut last_state = *self.halt_tx.borrow();
            ...
            tokio::select! {
                _ = interval.tick() => {
                    ...
                    if current != last_state {
                        ...
                        if current {
                            tracing::error!(
                                "HaltPollLoop: halt flag set — notifying {} receiver(s)",
...
mod tests {
    ...
    fn feed_heartbeat_resets_silence() {
        ...
        assert!(
            feed.silence_ms() < 100,
            "heartbeat should reset silence to near-zero"
        );
    }
    ...
    fn gas_spike_monitor_transitions_to_degraded() {
        ...
        let monitor =
            GasSpikeMonitor::new(layer.clone(), 100, fee.clone(), Duration::from_millis(10));
