...
//
// LA Blueprint Reorg Guard — spec §11.4.
//
...
// orphaned, the blueprint will not execute — the position monitor must
...
//
// ## Protection invariant (§11.3 interaction)
//
...
//   The map is pruned every block — entries older than STABILITY_WINDOW
//   blocks are removed.
...
/// (the chain stability window).  Spec §11.4.
...
struct SubmittedBlueprint {
    ...
    /// directly on this field — suppressed until that read is wired in.
    #[allow(dead_code)]
    block_hash: B256,
...
pub struct ReorgGuard {
    ...
    /// Broadcast channel for ReorgRiskEvents.  Capacity 64 — a reorg
...
struct ReorgGuardInner {
    /// Known block hashes: block_number → block_hash.
    /// Used to detect when a block's hash changes (reorg signal).
    known_blocks: HashMap<u64, B256>,
    /// Submitted blueprints: tx_hash → SubmittedBlueprint.
...
impl ReorgGuard {
    ...
    pub fn on_new_block(&self, block_number: u64, block_hash: B256) {
        ...
        inner
            .submitted
            .retain(|_, bp| bp.submitted_at >= prune_before);
        ...
        if let Some(&known_hash) = inner.known_blocks.get(&block_number) {
            if known_hash != block_hash {
                ...
                tracing::warn!(
                    ...
                    old_hash = %known_hash,
                    new_hash = %block_hash,
                    "Chain reorg detected — scanning for orphaned blueprints",
                );
        ...
        // No reorg — record/update the block hash
    ...
    /// sent to a relay.
    pub fn track_submission(&self, tx_hash: TxHash, block_number: u64, block_hash: B256) {
        let mut inner = self.inner.lock().expect("reorg guard mutex poisoned");
...
mod tests {
    ...
    fn same_hash_does_not_trigger_reorg() {
        ...
        let mut rx = g.reorg_events();
        g.on_new_block(100, hash(1));
        g.on_new_block(100, hash(1)); // same hash — no reorg
        assert!(rx.try_recv().is_err());
    }
    ...
    fn different_hash_triggers_reorg_and_marks_blueprints() {
        ...
        g.on_new_block(100, hash(1)); // first observation — no reorg
        g.on_new_block(100, hash(2)); // different hash — reorg!
    ...
    fn blueprint_at_different_block_not_marked() {
        ...
        g.on_new_block(100, hash(1));
        // Reorg at block 100 — blueprint at 99 must not be marked
    ...
    fn old_entries_pruned() {
        ...
        // Entry should be pruned — is_reorg_risk returns false for
        // unknown tx_hash
