...
//
// ChainId canonical chain identifiers for every chain the Omega Engine
// targets in v12.
//
// Spec references:
//   §1.1  — Phase table; Arbitrum is the primary execution chain for
//            Phases 1–3.  Ethereum mainnet enters in Phase 4 (MEV-OFA).
//   §7    — Dual-component gas model is Arbitrum-specific.
//   §11   — 80ms LA window is calibrated for Arbitrum's 250ms block time.
//   §12.2 — Arbitrum priority fee note; 500 gwei ceiling.
//   §12.3 — MEV-Boost builder blacklist applies to Ethereum L1 only.
//   §18   — CHI/GST gas token analysis — Ethereum L1 (Phase 4+) only;
//            not applicable on Arbitrum or Base.
//   §22.1 — Inter-crate dependency graph.  ChainId lives in omega-core
...
/// |----------|----------|-----------------------------------------------|
/// | Arbitrum | 42161    | PRIMARY — all Phases 1–3 execute here         |
/// | Ethereum | 1        | Phase 4 (MEV-OFA, L1 liquidations)            |
/// | Base     | 8453     | Defined; no strategies activated in v12        |
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ChainId {
    /// Arbitrum One — primary execution environment for Phases 1–3.
    /// 250ms block time, Arbitrum dual-component gas model (§7),
    /// direct sequencer submission (no MEV-Boost).
    Arbitrum = 42161,

    /// Ethereum mainnet — Phase 4 MEV-OFA and L1 liquidations.
    /// 12s block time, MEV-Boost + builder blacklist (§12.3),
    /// CHI/GST gas token analysis (§18).
    Ethereum = 1,

    /// Base — defined for future expansion; no v12 strategies.
    Base = 8453,
}
...
impl ChainId {
    ...
    ///
    /// Used by the LA tier monitor (§11) to compute the 80ms window
    /// budget relative to block production rate, and by the reorg guard
    /// (§11.4) to size the 60-block sequencer restart deduplication
    ...
    /// The 80ms LA execution window budget in blocks (§11).
    ///
    /// Arbitrum: 80ms / 250ms = 0.32 → rounds to 1 block (the engine
    ...
    pub fn sequencer_restart_window_blocks(self) -> u64 {
        match self {
            // Spec §11.3: 60 blocks ≈ 15 seconds on Arbitrum.
            ChainId::Arbitrum => 60,
            // Ethereum and Base: same 15-second wall-clock window,
            // scaled to their block times.
            ChainId::Ethereum => 2, // 2 blocks × 12s = 24s (closest ≥ 15s)
            ChainId::Base => 8,     // 8 blocks × 2s  = 16s
        }
    }

    /// Returns `true` when the Gas War Engine should apply the
    /// Arbitrum dual-component gas model (§7).
    ///
    /// `false` on Ethereum mainnet and Base — those chains use the
    ...
    /// (§12.3).  Arbitrum uses direct sequencer submission — no
    /// MEV-Boost — so the blacklist is irrelevant there.
    #[inline]
    ...
    /// applicable (§18).  Arbitrum and Base do not support EVM storage
    /// refunds.
    #[inline]
    ...
    /// (§errors.rs) and discard the associated blueprint.
...
///
/// Callers should record a `DropCode::WrongChainId` metric event (§errors.rs)
...
mod tests {
    ...
    fn unknown_chain_id_returns_err() {
        assert_eq!(ChainId::from_u64(999).unwrap_err(), UnknownChainId(999),);
    }

    #[test]
    fn la_window_blocks_arbitrum() {
        // 80ms / 250ms block time → 1 block (spec §11)
