...
//
// Validation layer for the Loss Attribution Engine (§13).
//
...
///
/// `code` is a `&'static str` — suitable as a Prometheus label without
/// heap allocation.
...
/// Canonical execution trace fragment for attribution (§13).
///
/// `tx_hash` is `TxHash` (= `B256`) — type-safe, zero-copy,
    ...
    fn validate(&self, trace: &ExecutionTrace) -> ValidationResult<()> {
        ...
        if trace.tx_hash == B256::ZERO {
            return Err(ValidationError::new(
                "ZERO_TX_HASH",
                "Transaction hash is zero — trace was not submitted on-chain",
            ));
        }
        ...
        if trace.gas_used == 0 {
            return Err(ValidationError::new(
                "ZERO_GAS_USED",
                "Gas used is 0 — trace appears to be a pre-execution estimate, \
...
impl Validator<PipelineLossEvent> for PipelineLossEventValidator {
    fn validate(&self, event: &PipelineLossEvent) -> ValidationResult<()> {
        ...
        if event.estimated_loss_usd.is_nan() {
            return Err(ValidationError::new(
                "NAN_LOSS",
                "estimated_loss_usd is NaN — invalid numerical state",
            ));
        }
        ...
        if event.estimated_loss_usd.is_infinite() {
            return Err(ValidationError::new(
                "INFINITE_LOSS",
                "estimated_loss_usd is infinite — oracle or calculation error",
            ));
        }
        ...
        if event.estimated_loss_usd < 0.0 {
            return Err(ValidationError::new(
                "NEGATIVE_LOSS",
                format!(
                    "estimated_loss_usd is negative ({:.6}) — \
...
mod tests {
    ...
    fn zero_loss_is_valid() {
        ...
        assert!(
            PipelineLossEventValidator.validate(&e).is_ok(),
