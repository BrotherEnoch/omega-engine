...
//
// Gas model checkpoint persistence (spec §13.2, fix I1).
//
// ## Spec §13.2
//
...
//   API: GET /api/v1/la/gas-model/checkpoints → list with version, win_rate,
//        sample_count
//
// ## File naming
//
...
// ## Durability
//
//   `save` calls `File::sync_all()` after writing — the checkpoint is a
...
/// Serialised state of the online gas model at a validation boundary (§13.2).
///
...
pub struct ModelCheckpoint {
    ...
    /// Holdout win rate at the time this checkpoint was validated (§13.1).
    /// The revert threshold check compares against this field.
    pub win_rate: f64,

    /// Full fee multiplier map at checkpoint time.
    /// Restored verbatim on revert (§13.1 fix C1).
    ...
    /// Used by the ceiling escalation path (§13.3) to compare against
...
/// file — corrupt old versions are skipped (logged at WARN) until a
...
pub fn load_latest(dir: &Path) -> Result<Option<ModelCheckpoint>, CheckpointError> {
    ...
    // Sort descending — try highest version first
        ...
        match load_one(&path) {
            ...
            Err(e) => {
                tracing::warn!(
                    version = version,
                    error   = %e,
                    "Skipping corrupt checkpoint — trying older version",
                );
            }
        }
    }
...
/// Lightweight metadata for the checkpoint listing API (§17.2).
        ...
        match fs::remove_file(&path) {
            ...
            Err(e) => {
                tracing::warn!(
                    ...
                    path = %path.display(),
                    error = %e,
                    "Failed to prune checkpoint — continuing",
                );
            }
        }
    }
