...
//
// Attribution pipeline observability dashboard (spec §16, §17.2).
//
// ## Spec §16
//
...
//     GET /api/v1/la/gas-model/checkpoints  — checkpoint listing
//     GET /api/v1/la/gas-model/ceiling-status — escalation state
//
//   The dashboard serves as the in-process aggregator that backs these
//   endpoints.  It is NOT async — snapshots are taken by the control-
...
//   removed — ceiling escalation tracking is now in `ceiling_escalation.rs`
...
//   values even at a 99% success rate (e.g. 99 valid / 1 rejected →
//   score = 0.99 - 0.01 = 0.98, which is fine, but 50/50 → 0.0 instead
...
pub struct DashboardSnapshot {
    ...
    /// Current ceiling escalation state (§13.3, §17.2).
...
///
/// Thread safety: `AttributionDashboard` is NOT `Send + Sync` — it is
...
impl AttributionDashboard {
    ...
    pub fn record_valid_event(&mut self, event: &PipelineLossEvent, code: LossCode) {
        ...
        *self.loss_code_counts.entry(code.to_string()).or_insert(0) += 1;
    }
    ...
    /// Update the ceiling escalation state (§13.3, §17.2).
    ...
    ///
    /// Cheap to call — all fields are computed from cached counters.
    ...
    ///
    /// Threshold: success rate > 0.75 (§16 health criteria).
...
mod tests {
    ...
    fn success_rate_correct() {
        let mut d = AttributionDashboard::new();
        for _ in 0..75 {
            d.record_valid_event(&event(1.0), LossCode::LostGasLow);
        }
        for _ in 0..25 {
            d.record_rejected_event("NAN_LOSS");
        }
