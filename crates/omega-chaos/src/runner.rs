...
//
// ChaosRunner — executes chaos scenarios and produces a structured report.
//
// ## Execution model
//
...
impl ChaosReport {
    fn from_results(
        ...
        let passed = results
            .iter()
            .filter(|r| r.outcome == ScenarioOutcome::Pass)
            .count();
        let failed = results
            .iter()
            .filter(|r| r.outcome == ScenarioOutcome::Fail)
            .count();
        let skipped = results
            .iter()
...
impl ChaosRunner {
    ...
    async fn run_scenario(&self, config: &ScenarioConfig) -> ScenarioResult {
        ...
        let obs_strings: Vec<String> = self
            ...
            .map(|o| {
                format!(
                    "[{}] {}",
                    if o.expected { "OK" } else { "UNEXPECTED" },
                    o.message
                )
            })
            .collect();
    ...
    // ── Expected response checking ────────────────────────────────────────

    async fn wait_for_expected_response(&self, id: ScenarioId, deadline: Duration) -> bool {
    ...
    fn check_expected_response(&self, id: ScenarioId) -> bool {
        ...
        match id {
            ...
            ScenarioId::SequencerDown | ScenarioId::SequencerRestart => {
                ...
                if ok {
                    self.target
                        .observe("ExternalData Degraded on sequencer fault", true);
                }
                ok
            }
            ...
            ScenarioId::FlashCrash => {
                ...
                if ok {
                    self.target
                        .observe("Risk layer transitioned on flash crash", true);
                }
                ok
            }
            ...
            ScenarioId::DagCycle => {
                let has_cycle = self
                    .target
                    .has_fault(|f| matches!(f, FaultKind::DagCycle { .. }));
                if has_cycle {
                    self.target
                        .observe("DagCycle fault active — MissDagCycle drop expected", true);
                }
                has_cycle
            }
            ...
            ScenarioId::ZkProofDelay => {
                ...
                if ok {
                    self.target
                        .observe("Zk layer Degraded on proof delay", true);
                }
                ok
            }
            ...
            ScenarioId::RevmCacheStale => {
                ...
                if ok {
                    self.target
                        .observe("EIL Degraded on revm cache staleness", true);
                }
                ok
            }
            ...
            ScenarioId::FlashloanLiquidity => {
                ...
                if ok {
                    self.target
                        .observe("Flashloan Degraded on zero liquidity", true);
                }
                ok
            }

            ScenarioId::HighCompetition => {
                // Competition is not a health fault — verify all layers stay operational.
                ...
                if operational {
                    self.target
                        .observe("All layers operational under high competition", true);
                }
                operational
            }
            ...
            ScenarioId::RpcRateExhaust => {
                ...
                if ok {
                    self.target
                        .observe("ExternalData Degraded on RPC exhaustion", true);
                }
                ok
            }
        }
    ...
    fn fault_for(&self, id: ScenarioId) -> FaultKind {
        ...
        match id {
            ScenarioId::OracleStale => {
                ...
                FaultKind::OracleStale {
                    feed: "chainlink_eth".into(),
                    stale_secs: 60,
                }
            }
            ...
            ScenarioId::DagCycle => FaultKind::DagCycle {
                cycle_description: "A→B→C→A".into(),
            },
            ScenarioId::ZkProofDelay => {
                self.target.health_layers[&LayerId::Zk].set_state(
                    HealthState::Degraded,
                    "ZK proof latency 200ms > 20ms budget",
                );
            ...
            ScenarioId::FlashloanLiquidity => {
                ...
                FaultKind::FlashloanDry {
                    provider: "aave_v3".into(),
                }
            }
            ScenarioId::HighCompetition => FaultKind::HighCompetition { win_pct: 95.0 },
            ScenarioId::RpcRateExhaust => {
                self.target.health_layers[&LayerId::ExternalData]
