...
//
// Chaos scenario definitions — 14 scenarios covering all system fault classes
// (spec §9).
//
// ## Purpose
//
...
//
// ## The 14 scenarios (spec §9)
//
//   S1  OracleStale         — primary oracle stops updating for 45+ seconds
//   S2  OracleDiverge       — Chainlink and Pyth diverge beyond 0.4%
//   S3  SequencerDown       — Arbitrum sequencer unresponsive for 10 blocks
//   S4  SequencerRestart    — sequencer returns after 10-block gap (double-spend risk §11.3)
//   S5  FlashCrash          — price drops 20% in one block
//   S6  GasSpike            — base fee jumps from 10 → 500 gwei
//   S7  RelayTimeout        — all relays stop responding (no submission path)
//   S8  DagCycle            — circular dependency injected into blueprint DAG
//   S9  ZkProofDelay        — ZK proof generation takes 200ms instead of <20ms
//   S10 HealthCascade       — SystemHealth → Halted cascades to all layers
//   S11 RevmCacheStale      — revm cache diverges from on-chain state
//   S12 FlashloanLiquidity  — flashloan provider reports 0 available liquidity
//   S13 HighCompetition     — 95% of opportunities are won by competitors
//   S14 RpcRateExhaust      — RPC budget saturated; rate limiter throttling 100%
//
// ## Scenario structure
//
...
/// Identifier for one of the 14 chaos scenarios (spec §9).
...
impl ScenarioId {
    /// All 14 scenario IDs in spec §9 order.
    ...
    /// Expected system response — what the engine MUST do under this fault.
    pub fn expected_response(self) -> &'static str {
        match self {
            ...
            Self::SequencerDown => {
                "Pause relay submissions; Health→Degraded; auto-resume on reconnect"
            }
            Self::SequencerRestart => "Dedup guard prevents double-spend; resume within 60 blocks",
            Self::FlashCrash => {
                "Emit MissFlashCrash; halt LA scoring; recover when price stabilises"
            }
            Self::GasSpike => "Emit MissGasSpike; pause aggressive/emergency bundles",
            Self::RelayTimeout => {
                "Relay layer→Degraded; retry with backoff; no blueprint dropped silently"
            }
            Self::DagCycle => "Emit MissDagCycle; drop blueprint; DAG integrity maintained",
            Self::ZkProofDelay => "ZK layer→Degraded; queue blueprints; no timeout panic",
            ...
            Self::FlashloanLiquidity => {
                "MissFlashloan drop; no submission attempt; try next provider"
            }
