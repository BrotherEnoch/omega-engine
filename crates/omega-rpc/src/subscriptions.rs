...
//     2. Opens a WebSocket subscription via on_builtin (wss:// routing is
//        correct now that transport-ws + pubsub are compiled in workspace).
//     3. Publishes parsed events to the appropriate tokio broadcast channel.
//     4. Reconnects on error with exponential backoff.
//
// ## Streams
//
//   run_pending_tx_stream       — eth_subscribe("newPendingTransactions")
//   run_lending_protocol_stream — eth_subscribe("logs") for Aave v3,
//                                 Compound, Morpho, Euler v2
//   run_dex_sync_stream         — eth_subscribe("logs") for AMM Sync events
//   run_fee_oracle_stream       — eth_subscribe("newHeads") for base fee
//   run_mev_share_stream        — HTTP SSE from mev-share.flashbots.net
...
/// Build a WS provider from a URL string using on_builtin.
/// Requires transport-ws + pubsub features — guaranteed by workspace Cargo.toml.
async fn ws_provider(ws_url: &str) -> anyhow::Result<impl Provider> {
    ProviderBuilder::new()
        .on_builtin(ws_url)
        .await
...
/// Runs indefinitely; reconnects on error.  Spawn as a Tokio task.
...
///
/// Uses HTTP long-polling with `reqwest`.  Reconnects with exponential
/// backoff on SSE drop.
            ...
            match serde_json::from_str::<serde_json::Value>(json_str) {
                ...
                Err(e) => {
                    tracing::warn!(
                        error = %e,
                        raw   = json_str,
                        "Failed to parse MEV-Share payload",
                    );
                }
            }
        }
