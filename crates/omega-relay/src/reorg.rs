...
//! L2 reorg guard for LA blueprints during the 80 ms submission window (§11.4, V4).
...
/// Ethereum transaction hash — 32 bytes, hex-encoded with `0x` prefix.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct TxHash(pub String);

/// Chain stability window before rescoring a reorg-risk blueprint (§11.4).
...
pub enum BlueprintState {
    ...
    Submitted {
        /// Canonical block number at which the blueprint was first submitted.
        submitted_at_block: u64,
    },
    /// Submission block was orphaned — blueprint will not execute.
    ReorgRisk {
        /// Orphaned canonical block that invalidated the submission.
        orphaned_block: u64,
    },
    /// Blueprint included and confirmed.
    Included {
        /// Canonical block in which the blueprint was included.
        inclusion_block: u64,
    },
}
...
impl LaReorgGuard {
    ...
    /// A reorg orphaned `orphaned_block`. Moves affected blueprints to `ReorgRisk`.
...
mod tests {
    ...
    fn reorg_emits_event_with_correct_rescore_block() {
        ...
        guard.on_new_block(200, hash(20));
        ...
        assert_eq!(event.rescore_at_block, 200 + STABILITY_WINDOW_BLOCKS);
    }
    ...
    fn same_block_hash_does_not_trigger_reorg() {
        ...
        guard.on_new_block(100, hash(1));
        guard.on_new_block(100, hash(1));
        assert!(rx.try_recv().is_err(), "no event for identical hash");
        assert!(matches!(
