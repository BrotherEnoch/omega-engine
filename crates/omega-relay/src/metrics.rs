...
//!
//! ## Spec references
//! - §11.2  Cascade ordering uses `la_ranked_relays()`
//! - §14.1  Address rotation reads per-relay `la_inclusion_rate` per address
//! - §14.2  Tie band = relays within 5% of best inclusion rate
...
impl RollingWindow {
    ...
    fn record(&mut self, included: bool) {
        let Some(slot) = self.window.get_mut(self.cursor) else {
            return;
        };
        *slot = included;
    ...
    fn rate(&self) -> f64 {
        ...
        let included = self
            .window
            .get(..self.filled)
            .map_or(0, |window| window.iter().filter(|&&b| b).count());
...
/// Snapshot of a relay's current LA inclusion rate — used for cascade ordering.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct RelayRateSnapshot {
    /// Relay identity associated with this snapshot.
    pub relay: RelayName,
    /// Rolling-window LA inclusion rate in [0.0, 1.0].
    pub la_rate: f64,
    /// Total number of submissions observed for this relay/address pair.
    pub total_submitted: u64,
    /// Total number of successful inclusions observed for this relay/address pair.
...
/// (required for carryover seeding in §14.1).
pub struct LaRelayMetrics {
    /// `(relay_name, address)` → metrics
    ...
    window_size: usize,
    /// Active execution address — updated on rotation.
    active_address: RwLock<ExecutionAddress>,
}

impl LaRelayMetrics {
    /// Create a new relay metrics store with a rolling window and initial active address.
    ...
    /// Seed a new address with a pre-computed rate (used during address rotation §14.1).
    pub fn seed_new_address(
        &self,
        relay: &RelayName,
    ...
    /// Used by cascade submission (§11.2) and non-cascade normal LA (§14.2).
    ...
    /// These form the tie band for randomized round-robin (§11.2, §14.2).
    pub fn tie_band(&self, tie_pct: f64) -> Vec<RelayRateSnapshot> {
        ...
        ranked
            .into_iter()
            .filter(|r| r.la_rate >= threshold)
            .collect()
    }

    /// Raw inclusion rate for a specific (relay, address) — used by reputation carryover.
    ...
    pub fn last_updated_age(&self, relay: &RelayName) -> Option<Duration> {
        ...
        self.inner
            .get(&key)
            .map(|m| m.read().last_updated.elapsed())
    }
}
...
mod tests {
    ...
    fn records_and_ranks() {
        ...
        assert!(matches!(
            ranked.as_slice(),
            [first, second, ..]
                if first.relay == RelayName::Flashbots && first.la_rate > second.la_rate
        ));
    }
