...
//! Governance audit log — §20.1
//!
//! Every governance action is appended to an in-memory append-only log.
//! The log cannot be cleared or truncated by any code path — structural
...
/// Append-only governance audit log (§20.1).
///
...
impl GovernanceAuditLog {
    ...
    pub fn actions(&self) -> &[GovernanceAction] {
        &self.actions
    }
    ...
    pub fn latest(&self) -> Option<&GovernanceAction> {
        self.actions.last()
    }
}
...
mod tests {
    ...
    fn append_increments_id() {
        ...
        let id1 = log.append(
            GovernanceActionType::ThresholdUpdate,
            "alice",
            "update 1",
            false,
            0,
        ...
        let id2 = log.append(
            GovernanceActionType::SignatureDeployed,
            "alice",
            "update 2",
            false,
            1,
    ...
    fn log_is_append_only_structurally() {
        ...
        let mut log = GovernanceAuditLog::new();
        log.append(
            GovernanceActionType::EmergencyPatch,
            "bot",
            "emergency",
            true,
            0,
