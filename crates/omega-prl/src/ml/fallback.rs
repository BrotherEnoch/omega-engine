...
//! Deterministic heuristic fallback — §16.3
//!
//! Used whenever the ML path is disabled (§17.2, §17.3).
//! Every heuristic is a simple, auditable weighted combination of feature
//! vector values — no trained weights, no opacity (§16.1).

use crate::features::extractor::{
    FeatureVector, F_BUNDLE_COMPLEXITY, F_BUNDLE_RETRY_COUNT, F_GAS_ESCALATION_VEL, F_HF_VELOCITY,
    F_LEAK_SUSPICION, F_ORACLE_MANIPULATION, F_RELAY_ACCEPT_BIAS, F_RELAY_DELAY_US,
    F_SEARCHER_AGGRESSION,
};
use crate::ml::inference::{
    InferenceResult, MODEL_GAS_WAR, MODEL_LIQUIDATION, MODEL_RELAY, MODEL_SEARCHER,
};

/// Deterministic heuristic fallback (§16.3, §17.2).
...
impl DeterministicFallback {
    ...
    // ── Heuristics — auditable, no trained weights ────────────────────────────
