...
//! Gas War Predictive Intelligence — §13

/// §13.2 — Gas war bid curve forecast.
#[derive(Debug, Clone)]
pub struct GasWarForecast {
    ...
    pub confidence: f32,
    /// §13.3: PRL may recommend emergency bundle but NEVER bypasses cap_gwei.
...
impl GasWarForecast {
    ...
    ///
    /// `cap_gwei` — governance ceiling.  Forecast NEVER recommends exceeding it.
    pub fn from_observed(recent_fees: &[u64], competitor_count: u16, cap_gwei: u64) -> Self {
        ...
        let inclusion_prob = (1.0 / (1.0 + competitor_count as f32 * 0.15)).clamp(0.0, 1.0);
        // Emergency only if very high velocity AND headroom below cap (§13.3).
...
mod tests {
    ...
    fn cap_gwei_always_respected() {
        ...
        assert!(
            forecast.expected_clearing_fee <= 500,
            "must respect cap_gwei; got {}",
            forecast.expected_clearing_fee
        );
    }
