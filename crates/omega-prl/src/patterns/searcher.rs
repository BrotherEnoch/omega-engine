...
//! Searcher Fingerprinting Engine — §9

/// §9.3 — Searcher behaviour fingerprint.
#[derive(Debug, Clone)]
...
impl SearcherFingerprint {
    ...
    /// Used for competitor identification (§9.4).
    pub fn similarity(&self, other: &Self) -> f32 {
        ...
        // Escalation curve L2 distance → similarity
        let esc_dist: f32 = self
            .escalation_curve
            .iter()
        ...
        // Latency similarity
        let lat_diff = (self.inclusion_latency_mean as f32 - other.inclusion_latency_mean as f32)
            .abs()
            / 10_000.0;
    ...
    /// §9.4 — Actionable when confidence ≥ 0.70 (§15.3).
    #[inline]
...
mod tests {
    ...
    fn identical_fingerprint_similarity_is_one() {
        ...
        assert!(
            (sim - 1.0).abs() < 1e-4,
            "identical fp must have similarity ~1.0, got {sim}"
        );
    }
    ...
    fn actionable_above_threshold() {
        let fp = SearcherFingerprint {
            fingerprint_id: 1,
            relay_affinity: [0; 16],
            escalation_curve: [0.0; 8],
            retry_pattern: [0; 16],
            inclusion_latency_mean: 0,
            confidence: 0.80,
        };
