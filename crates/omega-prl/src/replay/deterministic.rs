...
//! Deterministic WAL-backed replay — §18
//!
//! Replay MUST reproduce pattern outputs bit-for-bit (§18.2).
//! Same input stream → same outputs because:
//!   1. Feature extraction is pure and deterministic (§6.1).
...
/// WAL-backed deterministic replay archive (§18).
...
impl ReplayArchive {
    ...
    /// Append a live event to the WAL.  Called on every ingested event (§18.2).
    ///
    /// WAL write failures are logged but NEVER propagated (§17.2).
    #[inline]
    pub fn append(&self, event: &PatternEvent) {
        if let Err(e) = self.wal.append(event) {
            error!(error = %e, "PRL WAL write failed — replay gap possible");
        }
    }
    ...
    /// no shared mutable locks during replay — maximises throughput.
    pub async fn replay(&self, from_ts: u64, to_ts: u64) -> anyhow::Result<Vec<ConfidenceScore>> {
        ...
        // Each shard gets all events — it filters by relevance internally.
        let domains = [
        ...
        let thresholds = Arc::new(arc_swap::ArcSwap::from_pointee(ThresholdConfig::default()));

        // Parallel replay across domains using Rayon.
        let domain_scores: Vec<Vec<ConfidenceScore>> = domains
            .par_iter()
            .map(|&domain| replay_domain(domain, &events, Arc::clone(&thresholds)))
            .collect();
        ...
        result.sort_by_key(|score| score.pattern_id.0);
    ...
    pub fn wal(&self) -> &EventWal {
        &self.wal
    }
}

/// Replay one domain shard — isolated, no shared state.
fn replay_domain(
...
mod tests {
    ...
    fn empty_event_list_returns_empty() {
        let thresholds = Arc::new(arc_swap::ArcSwap::from_pointee(ThresholdConfig::default()));
        let scores = replay_domain(PatternDomain::GasWar, &[], thresholds);
