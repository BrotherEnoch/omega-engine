...
//
// Per-asset, per-protocol liquidation bonus oracle (spec §11, §12).
//
// ## Purpose
//
//   The liquidation bonus determines the maximum gross revenue from an LA
//   opportunity.  The adaptive gas cap formula (§12.2) uses it to set the
//   priority fee ceiling:
//
//     base_cap = (bonus_eth × 1e9 × 0.05) / GAS_PER_BUNDLE
//
...
//               collateral absorbed.  Discount ~5–8% depending on asset.
//               Fetched via the Comet contract's `getAssetInfo`.
//
//   Morpho Blue: Derived from the market's LLTV.
//               bonus ≈ 1 / LLTV − 1 (inverse liquidation value margin).
...
fn default_bonus_bps(protocol: LendingProtocol, asset: &str) -> u16 {
    match (protocol, asset) {
        ...
        // Morpho Blue — market-specific; use conservative estimate
        (LendingProtocol::Morpho, _) => 600,

        // Euler v2 — vault-specific discount
...
/// formula (§12.2).  Falls back to static defaults when live data is
/// not yet available.
#[derive(Debug)]
pub struct LaBonusOracle {
    chain_id: u64,
    cache: DashMap<BonusKey, BonusEntry>,
    /// Live entry TTL — entries older than this are considered stale and
...
impl LaBonusOracle {
    ...
    /// Convenience: bonus fraction (e.g. 500 bps → 0.05) for Aave v3.
    ...
    /// or missing — the background refresh task should re-fetch.
...
mod tests {
    ...
    fn stale_entry_falls_back_to_default() {
        ...
        // Stale immediately — should return default
        assert_eq!(oracle.aave_v3_bonus_bps("WETH"), 500);
    }
