...
//
// Shared types for the execution DAG (spec §9).
...
pub enum DagError {
    /// Admitting this blueprint would create a dependency cycle (§9).
...
pub struct DagConfig {
    ...
    /// Spec §4: target <1ms simulation latency; high-throughput path.
...
///
/// Used by the shadow scorecard `dag_eviction_rate` metric (§shadow).
#[derive(Debug, Clone, Serialize, Deserialize)]
