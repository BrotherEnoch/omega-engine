...
fn parse_strategy_filter(s: &str) -> Result<StrategyFilter, String> {
    match s.to_ascii_lowercase().as_str() {
        ...
        other => Err(format!("Unknown strategy filter '{other}' — use all|sa|msa|la")),
    }
}
...
async fn main() -> Result<()> {
    ...
    if args.days < 30 {
        tracing::warn!(days = args.days, "Replay window < 30 days — Phase 1 gate requires 30-day positive net EV");
    }
    ...
    tracing::info!(start_block, end_block = current_block, "Block range determined");
    ...
    for day in 1..=args.days {
        ...
        let gross    = day_opps.iter().map(|o| {
            if o.strategy == "SA" { o.size_eth * 0.0008 } else { o.size_eth * cfg.liquidation_bonus_fraction }
        }).sum::<f64>();
        let gas_cost = day_opps.iter()
    ...
    if !phase_gate_pass {
        tracing::error!(net_ev_eth = total_net_ev, "PHASE GATE FAIL: net EV is not positive — Phase 1 activation blocked");
        std::process::exit(1);
    }
...
mod tests {
    ...
    fn thirty_day_replay_positive_net_ev() {
        ...
        assert!(total_net_ev > 0.0, "30-day replay must produce positive net EV, got {total_net_ev}");
    }
