...
# Writes the corrected omega-rpc client.rs (fixes E0282 in tests::sample_block).
$ErrorActionPreference = 'Stop'

Write-Host 'Writing crates\omega-rpc\src\client.rs...'
$content_0 = @'
...
//
// ## Fix (this revision): E0282 in tests::sample_block
//
...
mod tests {
    ...
    fn sample_block(number: u64) -> Block {
        ...
        // `Block<Transaction, Header>` instantiation via the same
        // default-parameter mechanism, with no need to name either
        // parameter explicitly. See this file's module-level "Fix (this
        // revision): E0282 in tests::sample_block" note.
...
'@
Set-Content -Path 'crates\omega-rpc\src\client.rs' -Value $content_0 -Encoding UTF8 -NoNewline

Write-Host ''
Write-Host 'Verifying...'
$check = Select-String -Path 'crates\omega-rpc\src\client.rs' -Pattern 'let mut block: Block = Block::default' -Quiet
if ($check) { Write-Host '  OK: crates\omega-rpc\src\client.rs' } else { Write-Host '  MISSING: crates\omega-rpc\src\client.rs' -ForegroundColor Red }