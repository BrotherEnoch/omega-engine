...
# Writes the corrected crates/omega-oracle/src/twap.rs (fixes truncated LINK address + token0/token1 orientation).
$ErrorActionPreference = 'Stop'

Write-Host 'Writing crates\omega-oracle\src\twap.rs...'
$content_0 = @'
...
//
// ## Audit fix (RESOLVED this revision) — LINK pool address was malformed
//
...
// would be worse than a loud, immediate `cargo test` failure — a wrong-
...
// 0x82af49447d8a07e3bd95bd0d56f35241523fbab1", "Fee Tier: 0.3%" — i.e.
...
// revision) — token0/token1 orientation was backwards
//
// Uniswap V3 pools always assign `token0` to whichever of the pair's
// two addresses is numerically LOWER — enforced by the factory
...
// pool — the OPPOSITE of what this entry previously claimed ("LINK as
...
// "LINK per WETH" — the previous `token0_is_numerator: true` setting
...
// `1.0 / price_ratio` = token0/token1 = WETH per LINK — the correct
// orientation.
//
...
// existing decimals-based test still passes regardless of orientation —
// only a live price read would have come out inverted.
//
// NOT independently confirmed via an on-chain `token0()`/`token1()`
// call (this environment has no live RPC access) — confidence here
...
pub fn arbitrum_pools() -> &'static [(&'static str, &'static str, bool, u8, u8)] {
    &[
        ...
        // this file's module-level "Audit fix (RESOLVED)" note) — a
        ...
        //
        // Real token0/token1 order (WETH < LINK by address — see this
...
'@
Set-Content -Path 'crates\omega-oracle\src\twap.rs' -Value $content_0 -Encoding UTF8 -NoNewline

Write-Host ''
Write-Host 'Verifying...'
$check = Select-String -Path 'crates\omega-oracle\src\twap.rs' -Pattern '0x468b88941e7cc0b88c1869d68ab6b570bcef62ff' -SimpleMatch -Quiet
if ($check) { Write-Host '  OK: crates\omega-oracle\src\twap.rs' } else { Write-Host '  MISSING: crates\omega-oracle\src\twap.rs' -ForegroundColor Red }