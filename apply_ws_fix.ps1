...
# Writes the corrected ops/control-plane/src/ws.rs (fixes six wire-format test assertions).
$ErrorActionPreference = 'Stop'

Write-Host 'Writing ops\control-plane\src\ws.rs...'
$content_0 = @'
...
//
// ## Wire format (fixed this revision — see "Fix" note below)
//
//   `WsEvent` (the broadcast event stream) is INTERNALLY tagged, with
//   NO separate payload wrapper — `omega_control_contracts::ws::WsEvent`
...
//
//     { "kind": "<snake_case_variant>", "field_a": …, "field_b": … }
//
//   e.g. `WsEvent::ProfitSplit { .. }` serialises as
//     { "kind": "profit_split", "blueprint_hash": "0x1", "pil_share_wei": "…", … }
//   NOT as `{ "type": "profit_split", "payload": { … } }`.
//
//   The two one-shot auth-handshake frame types are tagged differently
//   but follow the SAME flattened (no-wrapper) shape — just with
//   `tag = "type"` instead of `tag = "kind"`:
//     - `WsClientFrame` (client → server): `{ "type": "auth", "token": "…" }`
//     - `WsAuthFrame` (server → client ack): `{ "type": "auth_ok", "rate_limit": …, "window_secs": … }`
//       or `{ "type": "auth_failed", "rate_limit": …, "window_secs": … }`
//   `negotiate_auth` below hand-builds these via `serde_json::json!` and
//   was already correct against this shape — only the broadcast-event
...
// already agree on.
//
// `WS_CHANNEL_CAPACITY` resolves via `crate::WS_CHANNEL_CAPACITY` — this
...
// — an assumption made without having seen
...
// `#[serde(tag = "kind", rename_all = "snake_case")]` — no `content`,
...
// which already passed against this real shape — only this file's
// tests were out of sync with it. Fixed all six affected tests
// (`config_reloaded_…`, `model_pause_changed_…`, `blacklist_reloaded_…`,
// `health_transition_…`, `profit_split_…`, `gas_model_reverted_…`) to
// assert `"kind":"<variant>"` instead of `"type":"<variant>"`, and
// removed the now-incorrect `"payload":{` wrapper assertions — each
...
mod tests {
    ...
    // `#[serde(tag = "kind", rename_all = "snake_case")]` — INTERNALLY
    // tagged, no `content` wrapper — so every serialised frame carries
...
'@
Set-Content -Path 'ops\control-plane\src\ws.rs' -Value $content_0 -Encoding UTF8 -NoNewline

Write-Host ''
Write-Host 'Verifying...'
$check = Select-String -Path 'ops\control-plane\src\ws.rs' -Pattern 'kind\":\"config_reloaded' -SimpleMatch -Quiet
if ($check) { Write-Host '  OK: ops\control-plane\src\ws.rs' } else { Write-Host '  MISSING: ops\control-plane\src\ws.rs' -ForegroundColor Red }