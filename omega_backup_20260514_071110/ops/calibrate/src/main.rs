ï»¿// ops/calibrate/src/main.rs
//
// Weekly threshold recalibration â€” Omega Engine.
//
// ## Purpose
//
...
//
//   reorg_threshold_blocks       â€” Maximum block depth to protect against.
//                                  Derived from: 7-day rolling 99th-pctile
//                                  observed reorg depth on this chain.
//                                  Spec Â§11.4: currently 60 blocks (Arbitrum).
//
//   oracle_latency_p95_ms        â€” Alert threshold for oracle feed staleness.
//                                  Derived from: 7-day p95 observed oracle
//                                  update latency per feed.
//
//   competition_neutral_score    â€” Win probability at which the model is
//                                  considered "neutral" (neither under- nor
//                                  over-bidding).  Derived from: 7-day median
//                                  LA win rate.
//                                  Spec Â§13: used as baseline_win_rate in
//                                  the ML model.
//
//   dynamic_min_profit_multiplierâ€” Multiplier applied to the base minimum
//                                  profit threshold.  Derived from: 7-day
//                                  average gas cost / liquidation bonus
//                                  ratio.
//
//   warm_price_move_threshold_bpsâ€” Warm-tier recompute trigger (Â§11.1).
...
pub struct CalibrationOutput {
    ...
    // â”€â”€ Reorg protection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    /// Maximum reorg depth observed in the lookback window (99th pctile).
    pub reorg_p99_depth_blocks:        u64,
    /// Recommended sequencer restart window.  Max(reorg_p99 Ã— 2, 10) blocks.
    pub reorg_threshold_blocks:        u64,

    // â”€â”€ Oracle latency â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Competition â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    /// 7-day median LA win rate (0.0â€“1.0).  Used as `baseline_win_rate`
    /// in the ML online learner (Â§13).
    pub competition_neutral_score:     f64,
    /// 7-day p25 LA win rate â€” lower bound; below this the model is
    /// systematically underperforming.
    pub competition_low_threshold:     f64,

    // â”€â”€ Min-profit multiplier â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    /// Ratio of mean gas cost to mean liquidation bonus.
    /// `dynamic_min_profit = base_min_profit Ã— dynamic_min_profit_multiplier`.
    pub dynamic_min_profit_multiplier: f64,

    // â”€â”€ Warm-tier price move â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
//
// In production, the observability layer (Â§16) writes structured NDJSON
...
fn compute_calibration(
    ...
    // â”€â”€ Reorg depth â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    let reorgs: Vec<ReorgObservation> = load_ndjson(&data_dir.join("reorgs.ndjson"));
    let reorg_depths: Vec<f64> = reorgs.iter()
        .map(|r| r.depth_blocks as f64)
        .collect();
    let reorg_p99 = if reorg_depths.is_empty() {
        tracing::info!("No reorg data â€” using chain default");
        defaults.reorg_threshold_blocks / 2
    } else {
        percentile(reorg_depths.clone(), 0.99) as u64
    };
    let reorg_threshold = (reorg_p99 * 2).max(10).max(defaults.reorg_threshold_blocks / 2);
    ...
    for s in &oracle_samples {
        per_feed.entry(s.feed.clone()).or_default().push(s.latency_ms);
    }
    ...
    // â”€â”€ Competition score â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    let competition: Vec<CompetitionOutcome> =
        load_ndjson(&data_dir.join("competition.ndjson"));

    let win_rates: Vec<f64> = competition.iter()
        .map(|c| if c.won { 1.0 } else { 0.0 })
        .collect();

    let neutral_score = if win_rates.is_empty() {
        tracing::info!("No competition data â€” using chain default");
    ...
    // â”€â”€ Dynamic min-profit multiplier â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    let gas_bonus: Vec<GasBonusSample> =
        load_ndjson(&data_dir.join("gas_bonus.ndjson"));

    let ratios: Vec<f64> = gas_bonus.iter()
    ...
    // min-profit multiplier = 2Ã— the mean gas-to-bonus ratio
    // (ensures we require at least 2Ã— gas cost as minimum profit)
    let min_profit_mult = (mean_ratio * 2.0).max(1.0).min(5.0);
    ...
    // â”€â”€ Warm-tier price move threshold â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    let price_moves: Vec<PriceMoveSample> =
        load_ndjson(&data_dir.join("price_moves.ndjson"));

    let move_values: Vec<f64> = price_moves.iter()
        .map(|p| p.move_bps)
        .collect();
    ...
    let warm_threshold_bps = if move_values.is_empty() {
        ...
        // Cap between 10 and 100 bps (spec Â§11.1 range)
...
mod tests {
    ...
    fn percentile_p95_correct() {
        ...
        // p95 index = ceil(100 * 0.95) - 1 = 95 - 1 = 94 â†’ value 95.0
    ...
    fn median_even_count() {
        ...
        // median at p50: ceil(4 * 0.5) - 1 = 2 - 1 = 1 â†’ value 2.0
        let m = median(vals);
    ...
    fn defaults_arbitrum_reorg_threshold() {
        ...
        assert!(d.reorg_threshold_blocks >= 30,
            "Arbitrum reorg threshold must be â‰¥ 30 blocks");
    }

    #[test]
    fn calibration_with_no_data_uses_defaults() {
        let dir = tempfile::tempdir().unwrap();
        // No data files â†’ pure defaults
        ...
        assert!(cal.competition_neutral_score > 0.0
            && cal.competition_neutral_score <= 1.0);
    ...
    fn calibration_with_reorg_data() {
        let dir = tempfile::tempdir().unwrap();
        // Write synthetic reorg data: 10 events, depths 1â€“10 blocks
        ...
        let cal = compute_calibration(ChainId::Arbitrum, dir.path(), 7);
        // p99 of [1..10] = 10; threshold = max(10*2, 10) = 20, but also â‰¥ 30 (default/2)
        assert!(cal.reorg_threshold_blocks >= 10,
            "threshold={}", cal.reorg_threshold_blocks);
    ...
    fn calibration_competition_win_rate_computed() {
        ...
        let mut f = std::fs::File::create(&path).unwrap();
        // 60 wins, 40 losses â†’ median win rate â‰ˆ 1.0/0.0 â†’ median is 1.0 for >50%
        // Actually: 60 true (1.0) + 40 false (0.0), median at p50 index 49 = 0.0
        // Wait â€” sorted: 40 zeros then 60 ones; index 49 (0-based) = 0.0
        ...
        let cal = compute_calibration(ChainId::Arbitrum, dir.path(), 7);
        // 70 wins (1.0), 30 losses (0.0) â€” sorted: 30 zeros then 70 ones
    ...
    fn min_profit_multiplier_clamped() {
        ...
        let mut f = std::fs::File::create(&path).unwrap();
        // gas/bonus = 3.0 â†’ multiplier = 3.0 * 2 = 6.0 â†’ clamped to 5.0
        let entry = serde_json::json!({
            "gas_cost_eth": 3.0,
            "bonus_eth": 1.0,
            "protocol": "aave_v3",
        ...
        let cal = compute_calibration(ChainId::Arbitrum, dir.path(), 7);
        assert!((cal.dynamic_min_profit_multiplier - 5.0).abs() < 1e-9,
            "multiplier should be clamped to 5.0, got {}", cal.dynamic_min_profit_multiplier);
    }
    ...
    fn warm_threshold_bps_clamped() {
        ...
        let cal = compute_calibration(ChainId::Arbitrum, dir.path(), 7);
        assert!(cal.warm_price_move_threshold_bps <= 100,
            "warm threshold must not exceed 100 bps, got {}", cal.warm_price_move_threshold_bps);
    }
}