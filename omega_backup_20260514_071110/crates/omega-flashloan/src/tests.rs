...
mod tests {
    ...
    use std::sync::Arc;
    use crate::{FlashloanError, FlashloanProvider, LiquidityRegistry,
                premium_wei, repayment_wei, select_provider,
                AAVE_V3_PREMIUM_BPS, BALANCER_PREMIUM_BPS, UNISWAP_V3_PREMIUM_BPS};
    ...
    // â”€â”€ Premium calculation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    #[test]
    fn aave_premium_9bps() {
        let amount = eth(10);
        let prem   = premium_wei(FlashloanProvider::AaveV3, amount);
        // 9 bps of 10 ETH = 0.009 ETH = 9_000_000_000_000_000 wei
    ...
    fn uniswap_premium_30bps() {
        ...
        // 30 bps of 10 ETH = 0.03 ETH = 30_000_000_000_000_000 wei
    ...
    // â”€â”€ LiquidityRegistry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Provider selection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn falls_back_to_aave_when_balancer_insufficient() {
        let reg = test_registry();
        reg.update(42161, FlashloanProvider::Balancer, addr(0xB0), eth(10), 1); // insufficient
        reg.update(42161, FlashloanProvider::AaveV3,   addr(0xA0), eth(100), 1);
    ...
    fn empty_registry_returns_none_available() {
        ...
        assert!(matches!(err, FlashloanError::NoneAvailable { best_available_wei: w, .. }
            if w == U256::ZERO));
    }
    ...
    fn error_maps_to_miss_flashloan_drop_code() {
        let err = FlashloanError::NoneAvailable {
            amount_wei: eth(1), chain_id: 42161, best_available_wei: U256::ZERO,
        };
        assert!(matches!(
            err.to_omega_error(),
    ...
    // â”€â”€ Calldata encoding â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    fn aave_calldata_has_correct_selector() {
        ...
        let expected_selector = &keccak256(
            b"flashLoanSimple(address,address,uint256,bytes,uint16)"
        )[..4];
    ...
    fn balancer_calldata_has_correct_selector() {
        ...
        let expected_selector = &keccak256(
            b"flashLoan(address,address[],uint256[],bytes)"
        )[..4];
        assert_eq!(&calldata[..4], expected_selector);
    }
    ...
    fn uniswap_calldata_has_correct_selector() {
        ...
        let expected_selector = &keccak256(
            b"flash(address,uint256,uint256,bytes)"
        )[..4];
        assert_eq!(&calldata[..4], expected_selector);
    }
    ...
    fn calldata_length_is_multiple_of_32_plus_4() {
        for provider in [
            ...
            // 4-byte selector + NÃ—32 bytes ABI data
            assert_eq!((cd.len() - 4) % 32, 0,
                "provider {provider}: calldata tail must be 32-byte aligned");
        }
    }

    // â”€â”€ Provider metadata â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    #[test]
    fn balancer_has_lowest_priority_number() {
        assert!(FlashloanProvider::Balancer.priority()
            < FlashloanProvider::AaveV3.priority());
        assert!(FlashloanProvider::AaveV3.priority()
            < FlashloanProvider::UniswapV3.priority());
    }
