...
//
// Versioned OFA rule registry (spec Â§8).
//
// ## Rule set versioning
//
...
//     - A rule set is "active" when its `activated_at â‰¤ Utc::now()`.
...
//       version is â‰¤ the current active version.
//
//   Governance path: L2 fast-approve (Â§5).  The operator submits a
...
pub enum RegistryError {
    #[error(
        "Downgrade rejected: new version {new} â‰¤ active version {active}. \
...
impl RuleRegistry {
    ...
    /// rule set's version is â‰¤ the current active version.
    ///
    /// Returns `Err(RegistryError::FutureActivation)` when
    /// `activated_at` is in the future â€” rule sets must not be
    ...
    /// registry was somehow constructed with no entries â€” this cannot
    ...
    pub fn history(&self) -> Vec<RuleSetEntry> {
        ...
        h.sort_by(|a, b| b.rule_set.version.cmp(&a.rule_set.version));
        h
    }
...
mod tests {
    ...
    fn downgrade_rejected() {
        ...
        assert!(matches!(err, RegistryError::VersionDowngrade { new: 3, active: 5 }));
    ...
    fn same_version_downgrade_rejected() {
        ...
        assert!(matches!(err, RegistryError::VersionDowngrade { new: 1, active: 1 }));
    }
    ...
    fn multiple_upgrades_active_is_highest() {
        ...
        reg.register(make_rule_set(7)).unwrap_err(); // 7 < 10 â†’ rejected
