ï»¿// crates/omega-compliance/src/lib.rs
// crates/omega-compliance/src/lib.rs
//
// omega-compliance â€” OFA compliance validation library (spec Â§8).
//
// ## Purpose
//
//   Order Flow Agreements (OFA) require that bundles which include
//   user-initiated order flow:
//     1. Have explicit consent from the order's originator (Â§8).
//     2. Protect the user from slippage beyond the agreed bound (Â§8).
//     3. Carry a valid, unexpired order signed by the originator (Â§8).
//
//   Blueprints that set `ofa_compliant = true` MUST pass all three
//   checks before relay submission.  The compliance layer is the
//   authoritative gate â€” relays may additionally enforce their own
//   checks, but the engine never submits a non-compliant OFA bundle.
//
// ## Architectural role (Â§22.1)
//
//   omega-compliance â† omega-core
//
