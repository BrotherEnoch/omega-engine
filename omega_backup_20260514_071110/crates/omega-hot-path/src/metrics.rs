...
impl HotPathMetrics {
    ...
    pub fn snapshot(&self) -> HotPathMetricsSnapshot {
        HotPathMetricsSnapshot {
            ...
            histogram:           std::array::from_fn(|i| {
                self.histogram[i].load(Ordering::Relaxed)
            }),
        }
    }
    ...
    pub fn reset(&self) {
        ...
        for bucket in &self.histogram { bucket.store(0, Ordering::Relaxed); }
        if let Ok(mut g) = self.last_success_at.lock() { *g = None; }
    }
    ...
    fn add_profit(&self, profit: U256) {
        let as_u128 = if profit > U256::from(u128::MAX) { u128::MAX }
                      else { profit.to::<u128>() };
...
mod tests {
    ...
    fn miss_rate_correct() {
        let m = HotPathMetrics::new();
        for _ in 0..80 { m.record_success(300, U256::from(1_000_u64)); }
        for _ in 0..20 { m.record_miss(); }
    ...
    fn p95_latency_requires_min_20_observations() {
        let m = HotPathMetrics::new();
        for _ in 0..19 { m.record_success(300, U256::from(1_u64)); }
        assert_eq!(m.p95_latency_us(), 0);
    }
    ...
    fn p95_latency_estimated_from_histogram() {
        let m = HotPathMetrics::new();
        for _ in 0..95 { m.record_success(200, U256::from(1_u64)); }
        for _ in 0..5  { m.record_success(1_500, U256::from(1_u64)); }
        let p95 = m.p95_latency_us();
        assert!(p95 <= 500, "p95={p95} should be â‰¤500Âµs with 95% under 250Âµs");
    }
