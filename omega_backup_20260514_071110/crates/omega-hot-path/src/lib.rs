ï»¿// crates/omega-hot-path/src/lib.rs
//
...
//
//   simulator.rs:
//     MicrotxSimulator::simulate(&self, bp, read_budget, metrics) â€” 3 args
...
pub use gate::{AdmissionResult, HotPathGate, MICROTX_GAS_LIMIT, MICROTX_MAX_READS, HOT_PATH_SLOTS};
...
impl HotPathRunner {
    pub fn new(config: HotPathConfig) -> (Self, mpsc::Sender<HotPathRequest>) {
        ...
        let runner = Self { gate, simulator, metrics, rx };
        (runner, tx)
    }
...
mod tests {
    ...
    async fn runner_processes_eligible_sa_blueprint() {
        ...
        tx.try_send(HotPathRequest { blueprint: bp, resp_tx }).unwrap();
        ...
        assert!(resp.result.is_ok(),
            "SA blueprint with valid gas should succeed: {:?}", resp.result);
    }
    ...
    async fn runner_rejects_cnry_blueprint() {
        ...
        assert!(resp.result.is_err(), "CNRY must be rejected at hot-path gate");
    }
    ...
    async fn runner_rejects_msa_blueprint() {
        ...
        assert!(resp.result.is_err(), "MSA (not hot_path_eligible) must be rejected");
    }
    ...
    async fn runner_rejects_gas_over_limit() {
        ...
        assert!(resp.result.is_err(), "SA with gas > MICROTX_GAS_LIMIT must be rejected");
    }
    ...
    fn constants_exported() {
        assert!(HOT_PATH_SLOTS > 0);
        assert!(MICROTX_GAS_LIMIT > 0);
        assert!(MICROTX_MAX_READS > 0);
    }
}