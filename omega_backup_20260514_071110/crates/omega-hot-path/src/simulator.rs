...
impl MicrotxSimulator {
    pub fn new(initial_block: u64) -> Self {
        Self {
            current_block: std::sync::Arc::new(
                std::sync::atomic::AtomicU64::new(initial_block),
            ),
        }
    }

    pub fn set_block(&self, block: u64) {
        self.current_block.store(block, std::sync::atomic::Ordering::Release);
    }
    ...
    pub fn simulate(
        ...
        if bp.simulator != Simulator::Revm {
            return Err(SimulationError::WrongSimulator { actual: bp.simulator });
        }
        ...
        let current = self.current_block.load(std::sync::atomic::Ordering::Acquire);
        ...
        let l2_cost_wei = gas_used as u128
            * bp.base_fee_at_creation as u128
            * 1_000_000_000;
...
mod tests {
    ...
    fn sim()     -> MicrotxSimulator { MicrotxSimulator::new(1_000_000) }
    fn metrics() -> HotPathMetrics   { HotPathMetrics::new() }

    #[test]
    fn successful_simulation_returns_revm_simulator() {
        let bp = make_bp(100_000, 10_000_000_000_000_u128, 2_000_000, Simulator::Revm);
    ...
    fn latency_is_recorded() {
        let bp = make_bp(50_000, 10_000_000_000_000_u128, 2_000_000, Simulator::Revm);
        let r  = sim().simulate(&bp, 8, &metrics()).unwrap();
