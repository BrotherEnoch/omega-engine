...
//
//   Pyth publishes a confidence interval (Â± spread) with each price.
//   We reject any price where confidence / price > MAX_CONFIDENCE_RATIO
//   (default 1% = 100 bps).  A wide confidence interval signals low
//   price certainty â€” treating such prices as stale prevents trading on
//   uncertain data.
//
// ## Staleness
//
...
//   We maintain a static mapping from token symbol â†’ Pyth price ID for
//   the tokens used by LA and SA on Arbitrum.
//
// ## Source
//   https://pyth.network/developers/price-feed-ids â€” Crypto feeds
...
/// Token symbol â†’ Pyth price feed ID (hex without 0x prefix).
...
impl CacheEntry {
    ...
    fn confidence_ok(&self) -> bool {
        if self.price_usd <= 0.0 { return false; }
...
impl PythOracle {
    ...
    /// `confidence_usd`: Â± confidence bound in USD.
    ...
    pub fn update(
        ...
        let entry = CacheEntry { price_usd, confidence_usd, publish_time, block_number };
        ...
        if !entry.confidence_ok() {
            tracing::warn!(
                ...
                "Pyth: wide confidence interval â€” treating as stale",
            );
        }
    ...
    pub fn read(&self, token: &str) -> Option<OraclePrice> {
        ...
        // Reject wide confidence â€” treat as unavailable
        if !entry.confidence_ok() {
            tracing::debug!(
                token,
                "Pyth: wide confidence interval â€” read returns None",
            );
            return None;
        }
...
mod tests {
    ...
    fn wide_confidence_returns_none() {
        let p = PythOracle::new(42161);
        // confidence = 20 USD on 1800 â†’ 1.1% > 1% threshold
    ...
    fn exact_1pct_confidence_is_acceptable() {
        let p = PythOracle::new(42161);
        // confidence = 18 â†’ 18/1800 = 1.0% exactly â‰¤ threshold
    ...
    fn stale_price_detected() {
        let p = PythOracle::new(42161);
        // publish_time 60s in the past â†’ stale at 45s threshold
