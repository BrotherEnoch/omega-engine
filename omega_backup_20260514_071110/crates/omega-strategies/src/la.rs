...
//
// Liquidation Arbitrage (LA) â€” Phase 3 strategy (spec Â§1.1, Â§11).
//
// ## Overview
//
//   LA targets undercollateralised lending positions on Aave v3,
//   Compound v3, Morpho, and Euler v2 on Arbitrum.  It sources capital
//   via a flashloan (Aave v3 flashloan provider), executes the
//   liquidation call, and returns the loan in the same transaction,
//   keeping the liquidation bonus as profit.
//
// ## Spec constraints (Â§1.1, Â§11)
//
//   Phase:           3 (LA)
//   Lane:            Normal (hot tier can use Microtx â€” see Â§11.1)
//   Simulator:       Anvil (full fork needed for flashloan accounting)
//   Gas budget:      600,000 L2 units
//   Hot-path:        true when HF < 1.01 (hot tier, Â§11.1)
//   OFA:             false
//   Confirmation:    12 blocks
//
// ## Health factor tiers (Â§11.1)
//
//   Hot    (HF < 1.01):  scored on every oracle update, immediate
//   Warm   (HF 1.01â€“1.05): batched every 200ms or on >0.5% oracle move
//   Cold/Archived: not handled here (low-priority background rescan)
//
// ## Flashloan (Â§11)
//
//   Provider: Aave v3 (address injected at construction).
//   Amount: 100% of position debt value.
//   Blueprint construction gates on `flashloan_feasible()`.
...
use omega_core::types::oracle::LaTier;
...
const LA_L1_DATA_GAS:     u64 = 3_200;   // ~200 bytes calldata Ã— 16
const LA_EXPIRY_BLOCKS:   u64 = 1;       // Â§11: must land in current block
const LA_SLIPPAGE_BPS:    u16 = 50;
const LA_CONFIRMATION:    u8  = 12;
/// Liquidation bonus fraction for scoring (10% typical on Aave v3).
const LA_LIQUIDATION_BONUS_FRAC: f64 = 0.10;
/// Proxy debt position size (5 ETH) for scoring when oracle data is absent.
const LA_PROXY_DEBT_ETH:  f64 = 5.0;
...
/// Liquidation Arbitrage â€” Phase 3, Normal lane (Â§1.1, Â§11).
pub struct LaStrategy {
    chain_id:           u64,
    nonce:              AtomicU64,
    bytecode_hash:      B256,
    contract_addr:      Address,
    /// Aave v3 flashloan provider address on the target chain.
...
impl LaStrategy {
    ...
    /// Whether this LA opportunity is in the hot tier (HF < 1.01, Â§11.1).
    fn is_hot_tier(health_factor_e18: U256) -> bool {
        LaTier::from_hf_e18(health_factor_e18) == LaTier::Hot
    }

    /// Compute net profit from a liquidation of `debt_wei` at the
    /// liquidation bonus rate.
    ///
    /// `bonus_frac` = liquidation bonus fraction (e.g. 0.10 for 10%).
    fn net_profit_after_gas(
        ...
        // Gross profit = debt Ã— bonus_frac
        let gross = U256::from(
        ...
        let l2_cost = U256::from(
            (LA_GAS_BUDGET as f64 * self.gas.l2_buffer_factor) as u64,
        )
        .saturating_mul(U256::from(l2_fee_gwei))
        .saturating_mul(U256::from(1_000_000_000_u64));

        let l1_cost = U256::from(
            (LA_L1_DATA_GAS as f64 * self.gas.l1_data_buffer_factor) as u64,
        )
    ...
    /// Encode the liquidation calldata.
    ///
    /// Encodes: selector + borrower + debt_token + collateral_token +
    ///          debt_amount + min_collateral_out
    fn encode_liquidation_calldata(
        ...
        let mut buf = [0u8; 32];
        debt_amount.to_big_endian(&mut buf);
        data.extend_from_slice(&buf);
        min_collateral_out.to_big_endian(&mut buf);
        data.extend_from_slice(&buf);
        Bytes::from(data)
    }
}
...
impl StrategyTrait for LaStrategy {
    ...
    /// Hot tier LA is hot-path eligible (Â§11.1).
    ...
    fn base_min_profit_wei(&self)  -> U256 {
        // 0.001 ETH minimum â€” flashloan premium must not erode margin
        U256::from(1_000_000_000_000_000_u64)
    }

    async fn score(&self, signal: &SignalState) -> Result<OpScore> {
        // In production: iterate hot/warm tier positions from the LA position
        // index and score each.  Here we use the proxy position (5 ETH debt).
        ...
        match self.net_profit_after_gas(
            ...
            Some((net, _)) => {
                // LA has higher competition probability than SA/MSA
    ...
    async fn build_blueprint(&self, signal: &SignalState) -> Result<ExecutionBlueprint> {
        ...
        let calldata = Self::encode_liquidation_calldata(debt_wei, net_profit);
        ...
        Ok(ExecutionBlueprint {
            ...
            flashloan_available:    debt_wei.saturating_mul(U256::from(2)), // proxy
            calldata,
            ...
            price_impact_bps:       None, // liquidation: no AMM slippage
            ofa_compliant:          false,
...
mod tests {
    ...
    fn make() -> Arc<LaStrategy> {
        LaStrategy::new(
            ...
            B256::from([0xAB; 32]),
            Address::ZERO,
            Address::from([0xAA; 20]), // flashloan provider
            &OmegaConfig::default(),
        )
    }
    ...
    fn sig(base_fee: u64) -> SignalState {
        SignalState {
            state_version: 1, chain_id: 42161,
            block_number: 3_000_000, base_fee_gwei: base_fee,
            l1_data_fee_gwei: 2,
    ...
    async fn blueprint_expires_in_one_block() {
        let sig = sig(5);
        let bp  = make().build_blueprint(&sig).await.unwrap();
        assert_eq!(bp.expiry_block, sig.block_number + 1);
    }
    ...
    fn hot_tier_detection() {
        // HF = 1.005 Ã— 10^18 â†’ not hot
        let e18 = 1_000_000_000_000_000_000_u128;
        let hf_hot  = U256::from(e18 + e18 / 1000);   // 1.001
        let hf_warm = U256::from(e18 + 5 * e18 / 100); // 1.05
        assert!(LaStrategy::is_hot_tier(hf_hot));
        assert!(!LaStrategy::is_hot_tier(hf_warm));
    }
}