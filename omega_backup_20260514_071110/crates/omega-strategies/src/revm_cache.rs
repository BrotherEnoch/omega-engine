ï»¿// crates/omega-strategies/src/revm_cache.rs
//
// EIL double-buffer revm state cache (spec Â§6).
//
// ## Problem
//
...
//   SIMULATION_STATE_MISMATCH (Â§13.4) losses.
//
...
//
//   Update SLA: < 50ms from new block arrival to cache ready (Â§6).
//   At 250ms Arbitrum block times this gives â‰¥ 4 full update cycles
//   before the next block.
//
// ## Staleness guard (Â§13.4 SIMULATION_STATE_MISMATCH prevention)
//
//   The spec Â§13.4 fix M3 requires the revm trust window to be reduced
...
///
/// Immutable after construction.  Shared via `Arc` â€” cloning is O(1).
///
/// ## Contents
///
...
pub struct RevmStateCache {
    ...
    /// Used for latency metrics (Â§16) â€” not for staleness checks.
...
impl RevmStateCache {
    ...
    /// Â§13.4; default 2 in normal operation).
    ///
...
/// Double-buffer EIL revm state cache manager (Â§6).
///
/// Thread-safe â€” `Arc<RevmCacheManager>` is shared between the oracle
...
pub struct RevmCacheManager {
    ...
    /// Reduced from 2 â†’ 1 after SIMULATION_STATE_MISMATCH (Â§13.4).
...
impl RevmCacheManager {
    ...
    pub fn new(trust_window_blocks: u64) -> Arc<Self> {
        // Sentinel snapshots at block 0 â€” will be stale immediately.
    ...
    ///
    /// O(1) â€” atomic load + Arc clone.  Never blocks.
    ...
    /// 2. Store the new snapshot into it (ArcSwap â€” non-blocking).
    ...
    /// continue safely â€” the old Arc is still live.
    ///
    ...
    pub fn update(&self, block_number: u64, base_fee_gwei: u64, slot_count: usize) {
        ...
        // Atomic flip â€” all new readers see the new snapshot from here
    ...
    /// event (Â§13.4, fix M3 corrective action).
    pub fn tighten_trust_window(&self) {
        ...
        tracing::warn!(
            "RevmCacheManager: trust window tightened to 1 block \
             (SIMULATION_STATE_MISMATCH corrective action Â§13.4)",
        );
    }
...
mod tests {
    ...
    fn update_makes_current_snapshot_non_stale() {
        ...
        // Block 100, trust=2 â†’ stale at 103+
    ...
    fn staleness_exact_boundary() {
        ...
        assert!(!snap.is_stale(52, 2));  // 52 > 52 â†’ false
        assert!(snap.is_stale(53, 2));   // 53 > 52 â†’ true
    }
}