ï»¿// crates/omega-strategies/src/registry.rs
//
// StrategyRegistry â€” maps StrategyId to a verified StrategyTrait implementation.
//
// ## Spec Â§8 â€” Strategy whitelist and bytecode verification
//
...
//      `active_phase >= strategy.phase_required()` (Â§1.1, Â§20).
//
// ## Immutability after deployment
//
//   The registry is append-only and cloned into an `arc_swap::ArcSwap<StrategyRegistry>`
//   at startup.  Governance upgrades (Â§13 versioned process) produce a new
...
pub enum RegistrationError {
    #[error("Strategy {0:?} is already registered â€” registry is append-only")]
...
impl StrategyRegistry {
    ...
    ///
    /// Called before simulation (Â§8).  Returns `Err(MissWhitelist)` on
    /// mismatch.
            ...
            Some(s) => {
                if s.expected_bytecode_hash() != blueprint_hash {
                    tracing::error!(
                        ...
                        "Bytecode hash mismatch â€” blueprint dropped (Â§8)",
                    );
...
/// be registered â€” they simply produce no blueprints until the phase gate
/// advances).
#[derive(Default)]
...
mod tests {
    ...
    impl StrategyTrait for FakeStrategy {
        fn strategy_id(&self)            -> StrategyId { self.id }
        fn lane(&self)                   -> Lane       { Lane::Normal }
        fn hot_path_eligible(&self)      -> bool       { false }
        fn gas_budget(&self)             -> u64        { 300_000 }
        ...
        fn encode_calldata(&self, _: &ExecutionBlueprint) -> Bytes { Bytes::new() }
    }
    ...
    fn register_and_get() {
        let reg = StrategyRegistryBuilder::new(3)
            .register(make_strategy(StrategyId::La, 0xAB)).unwrap()
            .build();
    ...
    fn phase_gate_blocks_higher_phase() {
        ...
        let reg = StrategyRegistryBuilder::new(1)
            .register(make_strategy(StrategyId::Mev, 0x01)).unwrap()
            .build();
        // Registered but gated â€” get must return None
    ...
    fn phase_gate_allows_equal_phase() {
        let reg = StrategyRegistryBuilder::new(3)
            .register(make_strategy(StrategyId::La, 0x01)).unwrap()
            .build();
        assert!(reg.get(StrategyId::La).is_some());
    }
    ...
    fn active_strategies_sorted_by_priority() {
        let reg = StrategyRegistryBuilder::new(4)
            .register(make_strategy(StrategyId::Sa,  0x01)).unwrap()
            .register(make_strategy(StrategyId::Mev, 0x02)).unwrap()
            .register(make_strategy(StrategyId::La,  0x03)).unwrap()
            .build();
    ...
    fn bytecode_verification_mismatch() {
        ...
        let reg = StrategyRegistryBuilder::new(3)
            .register(make_strategy(StrategyId::La, 0xAB)).unwrap()
            .build();
        let err = reg.verify_bytecode_hash(StrategyId::La, wrong_hash).unwrap_err();
    ...
    fn bytecode_verification_unregistered() {
        ...
        let err = reg.verify_bytecode_hash(StrategyId::La, B256::ZERO).unwrap_err();
        assert!(matches!(err.drop_code(), Some(DropCode::MissWhitelist)));
    }
}