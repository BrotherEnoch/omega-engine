...
//
// MEV-OFA / Backrun (MEV) â€” Phase 4 strategy (spec Â§1.1, Â§4, Â§8).
//
// ## Overview
//
...
//   OFA:          true â€” MUST route through OFA-compliant relays only
//   Min profit:   dynamic (from GasConfig + adverse selection discount)
//   Confirmation: 12 blocks
//
// ## Adverse selection guard (Â§8)
//
...
//   `encode_calldata` returns `blueprint.calldata` directly â€” MEV does
...
/// Backrun calldata is typically 2â€“3 hops; ~250 bytes Ã— 16.
...
/// that will move the market against the backrun position (Â§8).
...
/// MEV-OFA / Backrun strategy â€” Phase 4, Normal lane (Â§1.1, Â§4, Â§8).
...
impl MevStrategy {
    ...
    // â”€â”€ Gas model helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    /// Compute the dynamic minimum profit in wei using the dual-component
    /// gas model (Â§7).
    ///
    /// min_profit = (l2_gas Ã— base_fee Ã— l2_buf + l1_gas Ã— l1_fee Ã— l1_buf) Ã— 1e9
    ...
    ///
    /// Reads `signal_state_hash[0]` as a proxy proxy for the score â€”
    ...
    fn estimated_profit_wei(signal: &SignalState) -> U256 {
        // Proxy: 0.001â€“0.01 ETH depending on state version parity
        // Real: parsed from OracleSignal::payload["price_impact_wei"]
        let base: u128 = 1_000_000_000_000_000; // 0.001 ETH
...
impl StrategyTrait for MevStrategy {
    // â”€â”€ Static metadata â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    fn strategy_id(&self)  -> StrategyId { StrategyId::Mev }
    fn lane(&self)         -> Lane       { Lane::Normal }

    /// MEV is NOT hot-path eligible (Â§4).
    ...
    fn gas_budget(&self) -> u64 { MEV_GAS_BUDGET }

    fn base_min_profit_wei(&self) -> U256 {
        // 0.002 ETH baseline â€” must exceed competitive backrun costs
    ...
    ///
    /// Returns 0.0 when:
    ///   - The adverse selection score exceeds 0.5 (Â§8 guard)
    ///   - Estimated profit is below dynamic_min_profit
    ///
    /// Score formula:
    ///   raw_score = estimated_profit / (dynamic_min_profit Ã— 2)
    ///   adjusted  = raw_score Ã— (1 âˆ’ competition_prob)
    async fn score(&self, signal: &SignalState) -> Result<OpScore> {
        ...
        // Score: ratio of profit to 2Ã— min threshold, discounted by adverse
    ...
    // â”€â”€ build_blueprint â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    /// Build a MEV backrun blueprint.
    ///
    /// The blueprint sets `ofa_compliant = true` â€” the relay layer will
    ...
    async fn build_blueprint(&self, signal: &SignalState) -> Result<ExecutionBlueprint> {
        ...
        let mut bp = ExecutionBlueprint {
            ...
            ofa_compliant:           true,     // MUST use OFA-compliant relays (Â§8)
    ...
    // â”€â”€ simulate â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ encode_calldata â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    /// Return the calldata from the blueprint unchanged.
    ///
    /// MEV does not re-encode after simulation â€” the backrun calldata is
...
mod tests {
    ...
    // â”€â”€ Adverse selection guard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    #[tokio::test]
    async fn high_adverse_selection_skips() {
        let strategy = make_strategy();
        // hash_byte_0 = 255 â†’ adverse_score = 1.0 > 0.5
        let signal = test_signal(1, 255);
        let score  = strategy.score(&signal).await.unwrap();
        assert_eq!(score.score, 0.0, "high adverse selection must produce score 0");
    }

    #[tokio::test]
    async fn low_adverse_selection_proceeds() {
        let strategy = make_strategy();
        // hash_byte_0 = 100 â†’ adverse_score â‰ˆ 0.39 < 0.5
        let signal = test_signal(5, 100);
        let score  = strategy.score(&signal).await.unwrap();
        // State version 5 â†’ profit multiplier 6 â†’ â‰ˆ 0.006 ETH
        // min_profit at 10 gwei base = small â†’ should score > 0
        assert!(score.score > 0.0 || score.expected_profit == U256::ZERO,
            "low adverse selection should not be blocked: score={}", score.score);
    }

    // â”€â”€ blueprint construction â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    #[tokio::test]
    async fn blueprint_sets_ofa_compliant() {
        let strategy = make_strategy();
        // hash_byte_0 = 50 â†’ adverse â‰ˆ 0.196, profit should exceed min
        let signal = test_signal(9, 50);
        // Score first to confirm it should proceed
        let score = strategy.score(&signal).await.unwrap();
        if !score.should_proceed() {
            return; // profit below min at this fee level â€” skip
        }
        let bp = strategy.build_blueprint(&signal).await.unwrap();
        assert!(bp.ofa_compliant, "MEV blueprints must set ofa_compliant=true");
        assert_eq!(bp.strategy_id, StrategyId::Mev);
        assert_eq!(bp.simulator,   Simulator::Anvil);
        assert!(!bp.blueprint_hash.is_zero(), "blueprint_hash must be non-zero");
    }
    ...
    async fn blueprint_does_not_use_flashloan() {
        ...
        let signal   = test_signal(9, 50);
        let score    = strategy.score(&signal).await.unwrap();
        if !score.should_proceed() { return; }
        let bp = strategy.build_blueprint(&signal).await.unwrap();
        assert_eq!(bp.flashloan_amount, U256::ZERO,
    ...
    // â”€â”€ nonce monotonicity â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
