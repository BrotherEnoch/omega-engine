ï»¿// crates/omega-cross-chain/src/lib.rs
//
// omega-cross-chain â€” Cross-chain capability boundary for the Omega Engine.
//
// ## Status: EXPLICITLY DEFERRED (spec Â§11.5, Â§V2)
//
...
//
//   This is NOT a gap â€” it is an intentional architectural boundary.
//   The spec Â§11.5 closure note states:
//
...
//
//   1. `AtomicBridgeRequirement` â€” the concrete latency and security
//      requirements a bridge must satisfy before activation.
//
//   2. `BridgeFinality` â€” a measured or claimed finality reading from a
//      specific bridge.  Used by `AtomicBridgeRequirement::is_satisfied`
//      to evaluate candidate bridges.
//
//   3. `CrossChainCapability` â€” the runtime capability descriptor.
//      Always returns `CrossChainCapability::deferred()` in v12.
//      Exposes `is_available()` â†’ `false` so call sites can query
//      without hard-coding the deferral reason.
//
//   4. `DeferralRecord` â€” the structured audit record of why cross-chain
//      was deferred, matching spec Â§11.5.  Serialisable for the
//      control-plane API.
//
//   5. `ReEvaluationCriteria` â€” the checklist a future engineer must
//      satisfy to re-activate cross-chain.  Prevents re-activation
//      without a complete technical review.
//
// ## Dependency graph (Â§22.1)
//
//   omega-cross-chain â† omega-core, omega-relay
//
...
pub enum CrossChainError {
    /// Capability is deferred â€” the bridge latency requirement is not met.
    #[error(
        "Cross-chain LA deferred (spec Â§11.5): requires atomic bridge with \
    ...
    #[error(
        "Bridge '{bridge_name}' finality {actual_ms}ms exceeds limit \
         {required_ms}ms for chain pair {source_chain:?} â†’ {dest_chain:?}"
    )]
    FinalityExceeded {
        bridge_name:  String,
        actual_ms:    u64,
...
/// cross-chain LA can be activated (spec Â§11.5).
///
...
pub struct AtomicBridgeRequirement {
    ...
    ///
    /// Spec Â§11.5: "< 50ms" (conservative margin below the 80ms LA window).
...
impl AtomicBridgeRequirement {
    /// The v12 requirement set from spec Â§11.5.
    pub fn v12() -> Self {
        Self {
            max_finality_ms:              50,
    ...
    pub fn is_satisfied(
        ...
        if !finality.ethereum_equivalent_security && self.ethereum_equivalent_security {
            tracing::warn!(
                ...
                "Bridge does not claim Ethereum-equivalent security (Â§11.5)",
            );
        }
        if !finality.supports_atomicity && self.requires_atomicity {
            tracing::warn!(
                bridge = %finality.bridge_name,
                "Bridge does not support atomic cross-chain execution (Â§11.5)",
            );
        }
...
/// `#[derive(Deserialize)]` can satisfy the `'de: 'static` bound â€” serde's
...
impl DeferralRecord {
    /// The canonical v12 deferral record (spec Â§11.5).
    pub fn v12() -> Self {
        Self {
            spec_section: "Â§11.5, Â§V2".into(),
            reason: "Atomic cross-chain liquidation requires a bridge with finality \
                     ...
                     meets this requirement as of April 2026.".into(),
            re_activation_condition: "A production atomic bridge with < 50ms finality \
                     becomes available with Ethereum-equivalent security guarantees.".into(),
...
///
/// Fields are `String` for the same reason as `DeferralRecord` â€” serde's
...
impl ReEvaluationCriteria {
    ...
    ///
    /// All `completed` fields are `false` â€” they must be filled in by
            ...
            items: vec![
                ...
                ReEvaluationItem {
                    ...
                    description: "omega-strategies gains a CrossChainLA strategy variant \
                                  ...
                                  double-spend protection across chains.".into(),
                    completed: false,
                },
                ReEvaluationItem {
                    ...
                    description: "SequencerRestartHandler extended to cover cross-chain \
                                  ...
                                  source and destination chains simultaneously.".into(),
                    completed: false,
                },
                ReEvaluationItem {
                    id: "governance_proposal".into(),
                    description: "L3 governance proposal (48h timelock) submitted with \
                                  all above items complete and bridge audit attached.".into(),
                    completed: false,
                },
            ],
        }
    ...
    pub fn incomplete(&self) -> Vec<&str> {
        self.items.iter().filter(|i| !i.completed).map(|i| i.id.as_str()).collect()
    }
}
...
impl CrossChainCapability {
    ...
    pub fn deferred() -> Self {
        ...
        tracing::info!(
            ...
            spec_section = "Â§11.5",
            "Cross-chain LA deferred â€” no qualifying bridge available",
        );
    ...
    /// cross-chain â€” activation requires a governance proposal.
...
mod tests {
    ...
    fn deferred_error_has_correct_required_ms() {
        ...
        match err {
            CrossChainError::Deferred { required_ms, .. } => {
                assert_eq!(required_ms, 50, "v12 requires < 50ms finality (Â§11.5)");
            }
    ...
    fn qualifying_bridge_passes_requirement() {
        ...
        assert!(req.is_satisfied(&bridge, ChainId::Arbitrum, ChainId::Ethereum).is_ok());
    }
    ...
    fn slow_bridge_fails_requirement() {
        ...
        let err = req
            ...
            .unwrap_err();
        assert!(matches!(err, CrossChainError::FinalityExceeded { actual_ms: 200, .. }));
    }
    ...
    fn same_chain_is_rejected() {
        ...
        let err = req
            ...
            .unwrap_err();
        assert!(matches!(err, CrossChainError::SameChain { chain: ChainId::Arbitrum }));
    }
    ...
    fn criteria_item_count_matches_spec() {
        let criteria = ReEvaluationCriteria::v12();
        assert_eq!(criteria.items.len(), 7, "must have exactly 7 re-evaluation items");
    }
    ...
    fn deferral_record_references_correct_spec_section() {
        ...
        assert!(rec.spec_section.contains("Â§11.5"), "must cite Â§11.5");
        assert!(rec.spec_section.contains("Â§V2"),   "must cite Â§V2");
    }
    ...
    fn capability_evaluate_bridge_delegates_correctly() {
        ...
        assert!(cap.evaluate_bridge(&fast, ChainId::Arbitrum, ChainId::Ethereum).is_ok());
    }
}