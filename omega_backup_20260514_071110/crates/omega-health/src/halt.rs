ï»¿// crates/omega-health/src/halt.rs
//
// HaltFlag â€” system-wide emergency halt mechanism.
//
// Spec Â§3:
...
//   200ms SLA is met with â‰¥19Ã— polling margin.
//
...
/// L0 Health FSM reaches all loops within 200ms (spec Â§3).
///
/// All clones share the same underlying `AtomicBool` via `Arc` â€” there
...
pub struct HaltFlag {
    ...
    /// Stored in a separate `Arc<parking_lot::Mutex<Option<â€¦>>>` so
    ...
    /// `halt()` and `clear()` â€” never on the hot poll path.
...
impl HaltFlag {
    ...
    pub fn halt(&self, issuer: omega_core::LayerId, reason: &str) {
        ...
        // Record under lock â€” only on the slow halt path, never polled.
    ...
    /// governance clearance (Â§3).  Emits a tracing event at WARN level
    ...
    /// ordering â€” the minimum correct ordering to observe a `SeqCst`
    ...
    ///
    /// Acquires a mutex lock â€” do NOT call on the hot poll path.  Use
