...
//
// Lock-free bounded ring buffer for high-frequency event ingestion (Â§16).
//
// ## Design
//
//   The ring buffer is the ingest path for OmegaEvents emitted by every
//   layer.  Callers MUST NOT block â€” if the buffer is full, events are
...
pub struct EventRingBuffer {
    ...
    /// push and drain â€” the mutex is held only during the push (single
...
impl EventRingBuffer {
    ...
    pub fn push(&self, event: OmegaEvent) -> bool {
        ...
        if !ok {
            ...
            tracing::error!(
                overflow_count = n,
                "Observability ring buffer overflow â€” oldest event dropped",
            );
        }
        ok
    }
...
impl RingBufferSnapshot {
    ...
    pub fn is_healthy(&self) -> bool { self.overflow == 0 }

    /// Fraction of events that overflowed [0.0, 1.0].
    pub fn overflow_rate(&self) -> f64 {
        if self.total_in == 0 { return 0.0; }
...
mod tests {
    ...
    fn overflow_increments_counter_and_overwrites_oldest() {
        ...
        assert!(buf.push(health_event())); // slot 1 â€” full
    ...
    fn total_in_counts_all_pushes() {
        ...
        for _ in 0..7 { buf.push(health_event()); }
    ...
    fn total_out_counts_drained() {
        let buf = EventRingBuffer::new(10);
        for _ in 0..5 { buf.push(health_event()); }
        buf.drain();
    ...
    fn snapshot_overflow_rate_correct() {
        ...
        let snap = buf.snapshot();
        // 2 total_in, 1 overflow â†’ rate = 0.5
    ...
    fn fifo_order_preserved() {
        ...
        for (i, ev) in events.iter().enumerate() {
            if let OmegaEvent::BlueprintAdmitted { blueprint_hash, .. } = ev {
                assert!(blueprint_hash.ends_with(&i.to_string()),
