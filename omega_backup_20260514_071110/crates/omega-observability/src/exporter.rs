...
//
// OmegaExporter â€” async drain loop for the ring buffer (spec Â§16).
//
// ## Spec Â§16
//
...
//
//   The exporter never blocks producers â€” if it falls behind, the ring
...
pub struct ExporterConfig {
    ...
    /// Default 10ms â€” matches the hot-path polling interval (Â§3).
...
/// Async exporter â€” drains the ring buffer and writes events to the log stream.
///
...
impl OmegaExporter {
            ...
            tokio::select! {
                    ...
                    if overflow > stats.events_overflowed {
                        ...
                        if config.enable_internal_tracing && new_overflow > 0 {
                            tracing::warn!(
                                ...
                                "Ring buffer overflow â€” events lost",
                            );
                        }
                ...
                _ = shutdown.changed() => {
                    if *shutdown.borrow() {
                        ...
                        tracing::info!("Exporter shutdown â€” draining remaining events");
                        loop {
                            let drained = Self::drain_batch(
                                &buffer, &sampler,
                                config.drain_batch_size.max(1),
    ...
    fn drain_batch(
        ...
        for event in events.into_iter().take(batch_size) {
            // Sampling gate â€” LA events always pass (Â§16).
            ...
            match serde_json::to_string(&event) {
                Ok(json) => {
                    // Emit at INFO level â€” Filebeat/Fluentd ships this to ELK.
                ...
                Err(e) => {
                    ...
                    if do_trace {
                        tracing::error!(
                            ...
                            "Failed to serialise OmegaEvent â€” event dropped",
                        );
                    }
                }
            }
...
mod tests {
    ...
    async fn sampled_out_events_are_counted() {
        ...
        // GasModelReverted is always-sampled â€” use a non-always-sampled event.
