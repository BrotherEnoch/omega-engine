...
//
// Gas model ceiling escalation â€” spec Â§13.3 (fix I5).
//
// ## What this module is
//
// "Ceiling escalation" in v12 refers specifically to the behaviour when
// the ML fee multiplier reaches its 5.0Ã— upper bound and the model
// continues to receive `LostGasLow` events â€” meaning the engine is
// losing gas wars even when bidding at the maximum configured multiplier.
//
// The spec (Â§13.3) defines the following response:
//   1. Count consecutive `LostGasLow` events at the 5.0Ã— ceiling.
...
//      c. Pause the model â€” no further multiplier updates until governance
...
// (Â§16) samples for the `GET /api/v1/la/gas-model/ceiling-status` API
// endpoint (Â§17.2).
...
/// (`GET /api/v1/la/gas-model/ceiling-status`, Â§17.2).
///
...
pub struct CeilingEscalationState {
    ...
    /// Number of consecutive `LostGasLow` events at the 5.0Ã— ceiling
...
///
/// This struct is intentionally NOT responsible for pausing the model â€”
...
impl CeilingEscalationTracker {
    ...
    ///
    /// Returns `true` when the hit count has crossed `threshold` â€” the
    ...
    /// Snapshot of current escalation state for the API (Â§17.2).
...
mod tests {
    ...
    fn record_unpause_resets_all() {
        let mut t = CeilingEscalationTracker::new(100);
        for _ in 0..50 { t.record_ceiling_hit(&key()); }
        t.record_pause();
        t.record_unpause();
        let snap = t.snapshot(false);
