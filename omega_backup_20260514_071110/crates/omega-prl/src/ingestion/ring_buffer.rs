...
//! Lock-free single-producer single-consumer ring buffer â€” Â§5.1
//!
...
/// Cache-line-padded atomic counter â€” prevents false sharing between head
/// and tail when accessed by separate CPU cores.
#[repr(C, align(64))]
struct Padded(AtomicUsize, [u8; CACHE_LINE - std::mem::size_of::<AtomicUsize>()]);

impl Padded {
    const fn new(v: usize) -> Self {
        Self(AtomicUsize::new(v), [0u8; CACHE_LINE - std::mem::size_of::<AtomicUsize>()])
    }
}
...
impl<T: Send> LockFreeRingBuffer<T> {
    ...
    pub fn new(capacity: usize) -> Self {
        Self { inner: Arc::new(RingInner::new(capacity.max(2))) }
    }

    /// Actual slot count (power-of-two â‰¥ requested capacity).
    ...
    /// are live at any time â€” the type system enforces this via ownership.
    pub fn split(self) -> (Producer<T>, Consumer<T>) {
        let consumer_inner = Arc::clone(&self.inner);
        (
            Producer { inner: self.inner },
            Consumer { inner: consumer_inner },
        )
    }
}
...
impl<T: Send> Consumer<T> {
    ...
    pub fn drain_into(&self, out: &mut Vec<T>, limit: usize) {
        ...
        while drained < limit {
            match self.pop() {
                Some(item) => { out.push(item); drained += 1; }
                None       => break,
            }
        }
    }
...
mod tests {
    ...
    fn drain_into_correct() {
        let rb = LockFreeRingBuffer::new(8);
        let (tx, rx) = rb.split();
        for i in 0u32..5 { tx.push(i).unwrap(); }
        let mut out = Vec::new();
