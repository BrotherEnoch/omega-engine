...
//! Pattern event bus â€” ingestion pipeline entry point.
//!
...
//! â€” same compact byte-sequence encoding as `serde_bytes`, zero extra deps.
...
/// Discriminant for the event payload layout (Â§4.1).
///
...
/// Origin subsystem â€” used for source-aware feature weighting.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, serde::Serialize, serde::Deserialize)]
#[repr(u8)]
...
/// Dispatch priority â€” used by `EventBus` to order shard drain batches.
///
...
// delegate to serde's `serialize_bytes` / `deserialize_bytes` â€” identical
...
mod serde_payload {
    ...
    pub fn serialize<S: Serializer>(bytes: &[u8; N], s: S) -> Result<S::Ok, S::Error> {
        // Compact byte-sequence encoding â€” bincode emits a length-prefixed blob.
...
pub struct PatternEvent {
    ...
    pub ts_nanos:    u64,
    /// Event discriminant â€” determines payload interpretation.
    ...
    /// Number of valid bytes in `payload`. Must be â‰¤ `MAX_PAYLOAD_LEN`.
...
impl ShardQueue {
    ...
    /// (backpressure: low-priority events are shed under load per Â§5.2).
    #[inline]
...
/// Each shard worker drains its own queue independently â€” no cross-shard
...
impl EventBus {
    ...
    ///
    /// `shard_count` â€” number of independent shard queues (typically == CPU
    /// cores on the NUMA node).
    /// `ring_buffer_capacity` â€” total event slots across all shards; each
    /// shard gets `capacity / shard_count`.
    pub fn new(shard_count: usize, ring_buffer_capacity: usize) -> Self {
        let shard_count   = shard_count.max(1);
        let per_shard_cap = (ring_buffer_capacity / shard_count).max(64);
        let shards        = (0..shard_count)
            .map(|_| ShardQueue::new(per_shard_cap))
            .collect();
        Self {
            shards,
            shard_count,
...
mod tests {
    ...
    fn publish_routes_to_correct_shard() {
        ...
        bus.publish(make_event(EventType::OraclePriceUpdate));  // type 0 â†’ shard 0
        bus.publish(make_event(EventType::RelayInclusionResult)); // type 1 â†’ shard 1
