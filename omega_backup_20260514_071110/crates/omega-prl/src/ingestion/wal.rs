...
//! Append-only binary WAL â€” Â§18
//!
//! Requirements (Â§18.1, Â§18.2, Â§18.3):
//!   - Binary append-only â€” no in-place mutation ever
...
/// Append-only WAL for deterministic replay (Â§18).
pub struct EventWal {
    /// Directory containing segment files.
    base_dir:          PathBuf,
    /// Active segment writer â€” Mutex for multi-producer safety.
...
impl EventWal {
    ...
    pub async fn open(base_dir: &Path) -> anyhow::Result<Self> {
        ...
        Ok(Self {
            ...
            zstd_level:        1, // fastest â€” latency-optimised
        })
    }

    /// Append a `PatternEvent` to the WAL.
    ///
    /// Enforces monotonic timestamp ordering â€” events arriving out of order
    ...
    pub fn append(&self, event: &PatternEvent) -> io::Result<()> {
        ...
        let raw = bincode::serialize(event)
            .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;

        // Compress payload with zstd.
        let compressed = zstd::encode_all(raw.as_slice(), self.zstd_level)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e))?;

        // CRC32 over compressed bytes.
        let crc = crc32fast::hash(&compressed);

        let frame = WalFrame { ts_nanos: ts, data: compressed, crc32: crc };
        ...
        // Explicit flush on every write â€” WAL must be durable.
        w.flush()?;

        let total = self.bytes_written.fetch_add(
            (8 + frame_bytes.len()) as u64,
            Ordering::Relaxed,
        );

        // Segment rotation â€” drop writer lock before rotating.
    ...
    /// Returns frames in monotonic timestamp order â€” bit-for-bit reproducible.
            ...
            loop {
                ...
                if magic != WAL_MAGIC {
                    warn!(path = %seg_path.display(),
                        "WAL: unexpected magic â€” stopping replay");
                    break;
                }
                ...
                if actual_crc != frame.crc32 {
                    error!(
                        ...
                        actual   = actual_crc,
                        "WAL CRC32 mismatch â€” frame skipped"
                    );
                    continue;
                }

                // Timestamp window filter.
                if frame.ts_nanos < from_ts { continue; }
                if frame.ts_nanos > to_ts   { break;    }
