...
//! Sequencer & Reorg Pattern Detection â€” Â§12

/// Â§12.3 â€” Sequencer and reorg risk assessment.
#[derive(Debug, Clone)]
...
impl SequencerRiskScore {
    ...
    ///
    /// - `jitter_ratio`     â€” std_dev / mean of block intervals
    /// - `hash_instability` â€” fraction of recent blocks with changed hashes
    /// - `rpc_divergence`   â€” fraction of RPC responses with divergent state
    pub fn from_observed(
        jitter_ratio:     f32,
        hash_instability: f32,
        rpc_divergence:   f32,
    ) -> Self {
        let instability  = (0.5 * jitter_ratio
            + 0.3 * hash_instability
            + 0.2 * rpc_divergence)
            .clamp(0.0, 1.0);
    ...
    /// Â§12.4 â€” Whether to pre-emptively activate the 60-block dedup window.
    /// Integrates with `SequencerRestartHandler` (v12 Â§11.3).
    #[inline]
...
mod tests {
    ...
    fn high_instability_activates_dedup() {
        ...
        assert!(r.should_activate_dedup(),
            "restart_prob={}, confidence={}", r.restart_probability, r.confidence);
    }
}