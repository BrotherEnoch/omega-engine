...
//! Deterministic feature extraction â€” Â§6
//!
//! Requirements (Â§6.1):
...
// Fixed-size feature vector â€” no heap, cache-line aligned
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

/// Feature vector length â€” must be divisible by SIMD width (8 f32 lanes).
...
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Feature index constants (per Â§6.2 categories)
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// Lookup tables
//
// `f32::sqrt()` is not a `const fn` â€” const_fn_floating_point_arithmetic is
// not stabilised â€” so a bare `static [f32; 128] = { ... x.sqrt() }` is
...
fn norm_gas(gwei: u64, baseline_gwei: u64) -> f32 {
    if baseline_gwei == 0 { return 0.0; }
    ((gwei as f64 / baseline_gwei as f64) - 1.0).clamp(-1.0, 1.0) as f32
}

/// Normalise a latency (Âµs) into [0, 1] with 10ms ceiling.
#[inline]
...
#[inline]
fn norm_prob(p: f32) -> f32 { p.clamp(0.0, 1.0) }

// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// FeatureExtractor
// â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

/// Deterministic, allocation-free feature extractor (Â§6.1).
...
impl FeatureExtractor {
    ...
    /// Update rolling gas baseline (EWMA Î± = 0.05).
    #[inline]
    pub fn update_baseline_gas(&mut self, gwei: u64) {
        self.gas_baseline_gwei = (self.gas_baseline_gwei as f64 * 0.95
            + gwei as f64 * 0.05) as u64;
    }

    /// Update rolling block-interval baseline (EWMA Î± = 0.05).
    pub fn update_baseline_block_interval(&mut self, ns: u64) {
        self.block_interval_baseline_ns = (self.block_interval_baseline_ns as f64 * 0.95
            + ns as f64 * 0.05) as u64;
    }

    // â”€â”€ Per-type extractors â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    fn extract_oracle(&self, ev: &PatternEvent, fv: &mut FeatureVector) {
        let p = &ev.payload;
        // Payload: [price_raw(8)] [prev_price(8)] [update_interval_ms(4)]
        if ev.payload_len < 20 { return; }
        ...
        fv.set(F_ORACLE_UPDATE_FREQ,  norm_latency_us(update_ms as u64 * 1000));
        fv.set(F_ORACLE_MANIPULATION, norm_prob(if deviation > 0.05 { 0.8 } else { 0.0 }));
    }

    fn extract_relay(&self, ev: &PatternEvent, fv: &mut FeatureVector) {
        let p = &ev.payload;
        // Payload: [relay_id(4)] [included(1)] [latency_us(4)] [leak_suspected(1)]
        if ev.payload_len < 10 { return; }
        let included       = p[4] != 0;
    ...
    fn extract_loss(&self, ev: &PatternEvent, fv: &mut FeatureVector) {
        ...
        if ev.payload_len < 18 { return; }
        let gas_paid    = u64::from_le_bytes(p[1..9].try_into().unwrap_or([0; 8]));
        let gas_used    = u64::from_le_bytes(p[9..17].try_into().unwrap_or([0; 8]));
        let protocol_id = p[17] as f32 / 4.0; // normalise 0..4 â†’ [0,1]

        fv.set(F_GAS_MISCALC_RATE, if gas_used > gas_paid { 1.0 } else { 0.0 });
    ...
    /// Sequencer restart â€” no payload fields used; max latency signal.
