...
//! SIMD acceleration for feature vector operations â€” Â§6.3
//!
...
// `CmpGt` is a trait in `wide` â€” must be in scope for `f32x8::cmp_gt` to
...
const _: () = assert!(FEATURE_DIM % 8 == 0);
const LANES:  usize = 8;
...
pub fn count_above_threshold(fv: &FeatureVector, threshold: f32) -> u32 {
    ...
    for i in 0..CHUNKS {
        ...
        for v in mask { if v.to_bits() != 0 { count += 1; } }
    }
    count
}
...
/// Element-wise max of two vectors â€” useful for merging signal vectors.
#[inline]
...
mod tests {
    ...
    fn make_fv(val: f32) -> FeatureVector {
        ...
        for i in 0..FEATURE_DIM { fv.set(i, val); }
        fv
    }
    ...
    fn count_above_threshold_correct() {
        let mut fv = FeatureVector::zeroed();
        for i in 0..32 { fv.set(i, 0.9); } // first 32 above 0.5
