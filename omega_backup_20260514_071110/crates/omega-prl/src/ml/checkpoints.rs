...
//! Model checkpoint store â€” Â§16.5
//!
...
///
/// Thread-safe via `DashMap` â€” multiple shard workers can query simultaneously.
pub struct ModelCheckpointStore {
    /// model_name â†’ ordered Vec<ModelCheckpoint> (newest last).
...
impl ModelCheckpointStore {
    ...
    pub fn load(model_dir: &Path) -> anyhow::Result<Self> {
        let store = Self { checkpoints: dashmap::DashMap::new() };
        ...
        for name in [MODEL_RELAY, MODEL_GAS_WAR, MODEL_LIQUIDATION, MODEL_SEARCHER] {
            ...
            if found.is_empty() {
                ...
                warn!(model = name, "No ONNX file found â€” placeholder registered");
            }
        ...
        info!(models = store.checkpoints.len(), "ModelCheckpointStore loaded");
        Ok(store)
    }

    /// Empty store â€” used when starting in heuristic-fallback mode.
    pub fn empty() -> Self {
        Self { checkpoints: dashmap::DashMap::new() }
    }
    ...
    pub fn rollback_to(&self, version: u32) -> anyhow::Result<()> {
        ...
        for mut entry in self.checkpoints.iter_mut() {
            ...
            if !found {
                any_missing = true;
                warn!(model = entry.key(), version,
                    "No checkpoint at requested version");
            }
        ...
        if any_missing {
            anyhow::bail!(
                "One or more models have no checkpoint at version {}", version
            );
        }
...
fn compute_sha256(path: &Path) -> anyhow::Result<String> {
    ...
    // CRC32 placeholder â€” replace with sha2 crate in production.
