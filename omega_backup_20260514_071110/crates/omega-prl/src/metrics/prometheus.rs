...
//! PRL Prometheus metrics â€” Â§19.1
//!
//! Metric names match spec Â§19.1 exactly.
...
/// All PRL Prometheus metrics (Â§19.1).
...
impl PrlMetrics {
    pub fn new() -> anyhow::Result<Self> {
        ...
        let events_ingested = IntCounter::with_opts(
            Opts::new("prl_events_ingested_total", "Total events ingested by the PRL"),
        )?;
        ...
        let inference_latency_us = Histogram::with_opts(
            ...
            .buckets(vec![1.0, 5.0, 10.0, 20.0, 30.0, 40.0, 50.0, 75.0, 100.0, 200.0]),
        )?;
        registry.register(Box::new(inference_latency_us.clone()))?;

        let confidence_avg = Gauge::with_opts(
            Opts::new("prl_confidence_avg", "Average confidence across active patterns"),
        )?;
        registry.register(Box::new(confidence_avg.clone()))?;

        let queue_depth = IntGauge::with_opts(
            Opts::new("prl_queue_depth", "Current event queue depth across all shards"),
        )?;
        registry.register(Box::new(queue_depth.clone()))?;

        let replay_divergence_total = IntCounter::with_opts(
            Opts::new("prl_replay_divergence_total", "Total replay divergences detected"),
        )?;
        registry.register(Box::new(replay_divergence_total.clone()))?;

        let ml_fallback_total = IntCounter::with_opts(
            Opts::new("prl_ml_fallback_total", "Total ML-to-heuristic fallbacks"),
        )?;
        registry.register(Box::new(ml_fallback_total.clone()))?;

        let relay_anomaly_total = IntCounter::with_opts(
            Opts::new("prl_relay_anomaly_total", "Total relay anomaly detections"),
        )?;
    ...
    pub fn set_queue_depth(&self, depth: i64) { self.queue_depth.set(depth); }
    pub fn set_confidence_avg(&self, avg: f64) { self.confidence_avg.set(avg); }
    pub fn inc_pattern_match(&self, domain: &str) {
        self.pattern_matches.with_label_values(&[domain]).inc();
    }

    /// Emit an always-sampled observability event and update counters (Â§19.2).
    pub fn emit_always_sampled(
        ...
        match event {
            ...
            ObservabilityEvent::PatternModelReverted =>
                self.inc_pattern_match("model_revert"),
            _ => {}
        }
    }
}
