...
//! Replay divergence verifier â€” Â§18.2
//!
//! Compares live pattern outputs against a replay run bit-for-bit.
//! Any divergence triggers Â§17.3 DEGRADED state + alert.
...
pub enum DivergenceKind {
    CountMismatch      { live_count: usize, replay_count: usize },
    PatternIdMismatch  { live_id: u64, replay_id: u64 },
    ScoreMismatch      { live_score: f64, replay_score: f64 },
    ConfidenceMismatch { live_conf: f64, replay_conf: f64 },
}

/// Bit-for-bit verifier (Â§18.2).
...
impl ReplayVerifier {
    ...
    /// Score comparison uses `f64::to_bits()` â€” no floating-point tolerance.
    pub fn verify(
        ...
        if divergences.is_empty() { Ok(()) } else { Err(divergences) }
    }
}
...
mod tests {
    ...
    fn score(id: u64, s: f64, c: f64) -> ConfidenceScore {
        ConfidenceScore { pattern_id: PatternId(id), score: s,
            confidence: c, evidence: 1, age_ns: 0 }
    }
    ...
    fn pattern_id_mismatch() {
        ...
        let err    = ReplayVerifier::verify(&live, &replay).unwrap_err();
        assert!(matches!(err[0].kind, DivergenceKind::PatternIdMismatch { .. }));
    }
}