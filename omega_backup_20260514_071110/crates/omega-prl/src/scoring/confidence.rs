...
//! Confidence score structure â€” Â§15.1
//!
//! Every emitted pattern MUST expose all five fields:
//!   score, confidence, evidence, temporal_stability, freshness (Â§15.1).

use crate::patterns::signatures::PatternId;
use crate::scoring::decay::decay_confidence;

/// Â§15.1 â€” Complete confidence record for one pattern match.
...
impl ConfidenceScore {
    ...
    /// More evidence â†’ lower variance.  Used by ranking (Â§15.3).
    #[inline]
    pub fn variance_estimate(&self) -> f64 {
        if self.evidence == 0 { return 1.0; }
        1.0 / (self.evidence as f64).sqrt()
    }

    /// Freshness score [0, 1] â€” 1.0 for brand-new, approaches 0 as score ages.
    #[inline]
...
mod tests {
    ...
    fn variance_decreases_with_evidence() {
        let low  = ConfidenceScore { pattern_id: PatternId(0), score: 1.0,
            confidence: 0.9, evidence: 5,    age_ns: 0 };
        let high = ConfidenceScore { evidence: 1000, ..low };
        assert!(high.variance_estimate() < low.variance_estimate());
    }

    #[test]
    fn freshness_degrades_with_age() {
        let fresh = ConfidenceScore { pattern_id: PatternId(0), score: 1.0,
            confidence: 0.9, evidence: 1, age_ns: 0 };
        let stale = ConfidenceScore { age_ns: 10_000_000_000, ..fresh }; // 10s
        assert!(stale.freshness() < fresh.freshness());
    }

    #[test]
    fn is_actionable_threshold() {
        let s = ConfidenceScore { pattern_id: PatternId(0), score: 1.0,
            confidence: 0.85, evidence: 1, age_ns: 0 };
