...
//! PRL health FSM â€” Â§17.1
//!
//! State transitions are monotonically degrading (Healthyâ†’Degradedâ†’Limitedâ†’Halted)
...
impl PrlHealth {
    ...
    pub fn halt(&self, reason: &str) {
        ...
        error!(reason, "PRL HALTED â€” all advisory outputs disabled");
    }

    /// Governance-approved recovery to HEALTHY.
    pub fn recover(&self, approved_by: &str) {
        self.state.store(PrlHealthState::Healthy as u8, Ordering::SeqCst);
    ...
    fn transition_to(&self, new: PrlHealthState, reason: &str) {
        let old = PrlHealthState::from_u8(
            self.state.swap(new as u8, Ordering::SeqCst)
        );
        if old != new {
