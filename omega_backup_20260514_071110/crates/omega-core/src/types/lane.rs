ï»¿// crates/omega-core/src/types/lane.rs
// crates/omega-core/src/types/lane.rs
//
// Lane and Simulator enums.
//
// Lane drives the selection of simulation backend (Â§1.1, Â§4):
//   Microtx  â†’ revm  (zero-copy, <200k gas, Phase 1 SA hot-path)
//   Normal   â†’ Anvil (full EVM fork, MSA / LA / MEV)
//
...
/// Execution lane â€” determines throughput tier and simulation backend.
...
pub enum Lane {
    ...
    /// Used by Phase 1 SA and Phase 3 LA hot-path (Â§4, Â§11).
    Microtx,
...
/// (Â§13.4 SIMULATION_STATE_MISMATCH vs SIMULATION_GAS_MISCALC).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum Simulator {
    /// revm in-process execution.  Fast, deterministic, relies on the
    /// EIL double-buffer cache (Â§6).  Cache staleness produces
    /// SIMULATION_STATE_MISMATCH losses (Â§13.4).
    Revm,
    /// Anvil fork.  Full node state â€” no cache staleness risk.
    /// Higher latency (~50â€“200ms).  Used when Microtx lane is
