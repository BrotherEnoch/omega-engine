ï»¿// crates/omega-core/src/types/mod.rs
//
// Public type surface of omega-core.
//
// Visibility policy (Â§22.1):
//   Every type here is `pub` â€” omega-core is a pure-types crate with
//   no business logic.  Business logic lives in the crate that owns
//   the relevant specification section (omega-strategies for Â§11,
//   omega-gas-war for Â§12, omega-loss-attribution for Â§13, etc.).
//
//   The only exception is internal helpers that are pub(crate) â€” these
...
pub use health::{HealthState, LayerHealth, LayerId};
