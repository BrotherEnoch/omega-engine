ï»¿// crates/omega-core/src/types/strategy.rs
//
// Core strategy abstraction â€” the trait every Omega strategy implements.
//
// Spec references:
//   Â§1.1  â€” strategy phases and StrategyId
//   Â§4    â€” simulation backend selection (Lane / Simulator)
//   Â§7    â€” Arbitrum dual-component gas model (feeds into OpScore)
//   Â§11   â€” LA hot-path requirements (hot_path_eligible, gas_budget)
//   Â§19   â€” EV-weighted rollout (score â†’ OpScore)
//   Â§20   â€” phase gates (phase_required enforced at activation)
//
// Architecture note (Â§22.1):
...
/// simulation (Â§6, Â§13.4 SIMULATION_STATE_MISMATCH).
///
/// This struct is deliberately thin â€” it carries only the fields needed
...
pub struct SignalState {
    /// Monotonically increasing snapshot version from the EIL (Â§6).
    ...
    /// Fed into the dual-component gas model (Â§7).
    pub base_fee_gwei: u64,

    /// L1 data fee oracle reading in gwei at snapshot time.
    /// Arbitrum-specific â€” see Â§7, Â§12.2.
...
///
/// Used by the DAG dispatcher (Â§9) and the EV-weighted rollout (Â§19)
...
pub struct OpScore {
    /// Dimensionless score in [0.0, 1.0].  Zero â†’ skip.
    pub score: f64,

    /// Expected net profit in wei after all costs.
    pub expected_profit: U256,

    /// Estimated probability (0.0â€“1.0) that a competitor will win the
...
/// Engine (Â§13) compares `profit_net` against the blueprint's
...
pub struct SimResult {
    ...
    /// `false` â†’ the profit figures are invalid; this is a
    /// SIMULATION_EXECUTION_REVERT (Â§13.4).
    pub success: bool,
}
...
/// Core strategy interface â€” every Omega strategy implements this trait.
///
...
/// without depending on omega-strategies (Â§22.1).
///
/// ## Execution flow
///
/// ```text
/// oracle tick
///   â†’ score(signal)          [cheap, synchronous-ish, no blueprint alloc]
///   â†’ build_blueprint(signal) [allocates blueprint, fetches calldata]
///   â†’ simulate(blueprint)     [revm or Anvil fork]
///   â†’ relay submission        [Gas War Engine, Â§12]
/// ```
///
/// Canary strategies override only `score` and `is_canary` â€” they never
...
pub trait StrategyTrait: Send + Sync {
    ...
    /// Whether this strategy is eligible for the <1ms hot-path (Â§11.1).
    ...
    /// Enforced by the Gas War Engine before submission (Â§12).
    fn gas_budget(&self) -> u64;

    /// Base minimum profit threshold in wei, before dynamic adjustment
    /// (Â§7).  The dynamic_min_profit on a blueprint may be higher after
    ...
    /// Validated by the StrategyRegistry (Â§8) before simulation.
    fn expected_bytecode_hash(&self) -> B256;

    /// Returns `true` for the Canary strategy (Â§1.1).
    ///
    ...
    // â”€â”€ Execution pipeline â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    /// Score the current opportunity.
    ///
    /// Called on every relevant oracle tick.  MUST be fast â€” this is
    ...
    ///   2. Apply the dual-component gas model (Â§7).
