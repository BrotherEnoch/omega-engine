...
//
// ExecutionDag â€” blueprint scheduling DAG (spec Â§9).
//
// ## Spec Â§9 â€” DAG and dependency resolution
//
...
//      admitting the node.  A cycle â†’ `DropCode::MissDagCycle`.
//
//   2. SLOT CAPACITY
...
//      Exceeding capacity â†’ `DropCode::MissCapacity` (Microtx) or
...
//      (spec Â§9: "preemptive eviction").
//
// ## Data model
//
//   Nodes: each admits one `ExecutionBlueprint` identified by `blueprint_hash`.
//   Edges: directed dependency edges A â†’ B mean "A must complete before B".
...
//   these:
//     - duplicate hash            â†’ Cycle("duplicate: {hash}")
//     - missing dependency        â†’ Cycle("dep not found: {dep}")
//     - lane at capacity          â†’ LaneFull { lane, capacity }
//     - no eviction candidate     â†’ LaneFull { lane, capacity }
//
//   EvictionRecord uses String fields (evicted_strat, caused_by, drop_code)
//   rather than typed u8/Lane â€” priority values and lane are formatted as
...
/// The execution scheduling DAG (spec Â§9).
///
/// Held behind a `Mutex` in the orchestrator task.
pub struct ExecutionDag {
    config:                DagConfig,
    graph:                 StableDiGraph<DagNode, ()>,
    /// Fast lookup: blueprint_hash â†’ NodeIndex.
...
impl ExecutionDag {
    ...
    // â”€â”€ Admitting blueprints â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    ///
    /// CNRY blueprints skip slot capacity checks (Â§1.1).
    pub fn admit(
        &mut self,
        blueprint:    ExecutionBlueprint,
        dependencies: &[B256],
        ...
        // â”€â”€ 1. Duplicate check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // DagError has no Duplicate variant â€” map to Cycle with a descriptive
        // message so the orchestrator's to_omega_error() â†’ MissDagCycle path
        // is correct.
        ...
        // â”€â”€ 2. Dependency existence check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // DagError has no DependencyNotFound variant â€” map to Cycle.
        for dep in dependencies {
            if !self.index.contains_key(dep) {
                tracing::warn!(
                    blueprint_hash = %hash,
                    dep            = %dep,
                    "DAG dependency not found â€” blueprint rejected",
                );
                self.total_cycle_drops += 1;
        ...
        // â”€â”€ 3. Cycle detection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if is_cyclic_directed(&self.graph) {
            ...
            tracing::warn!(
                ...
                "DAG cycle detected â€” blueprint rejected (DropCode::MissDagCycle)",
            );
            // DagError::Cycle is a tuple variant: Cycle(String)
            return Err(DagError::Cycle(format!("{hash} creates a dependency cycle")));
        }

        // â”€â”€ 4. Slot capacity check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                match self.try_evict_for(
                    ...
                    Err(_) => {
                        ...
                        tracing::debug!(
                            ...
                            "DAG capacity full â€” blueprint rejected",
                        );
        ...
        // â”€â”€ 5. Commit â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Completing / removing nodes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    pub fn complete(&mut self, hash: B256) -> Vec<B256> {
        ...
        let successors: Vec<NodeIndex> = self.graph
        ...
        let unblocked: Vec<B256> = successors
            ...
            .filter_map(|&succ| {
                self.graph.node_weight(succ).map(|n| n.blueprint.blueprint_hash)
            })
            .collect();
    ...
    pub fn ready(&self) -> Vec<B256> {
        self.graph
            ...
            .filter_map(|idx| {
                self.graph.node_weight(idx).map(|n| n.blueprint.blueprint_hash)
            })
            .collect()
    }
    ...
    #[inline]
    pub fn microtx_count(&self) -> usize { self.microtx_count }

    #[inline]
    pub fn normal_count(&self) -> usize { self.normal_count }

    #[inline]
    pub fn node_count(&self) -> usize { self.graph.node_count() }

    pub fn evictions(&self) -> &VecDeque<EvictionRecord> { &self.evictions }

    /// Produce a serialisable snapshot for the observability API.
    pub fn snapshot(&self) -> DagSnapshot {
        let total_processed = self.total_admitted
            + self.total_cycle_drops
    ...
    fn try_evict_for(
        ...
        match candidate {
            Some((evicted_priority, evicted_hash, evicted_strat, node_idx)) => {
                ...
                // EvictionRecord uses String fields â€” format typed values
            ...
            None => {
                ...
                // No lower-priority candidate â€” lane is full with equal/higher priority nodes
