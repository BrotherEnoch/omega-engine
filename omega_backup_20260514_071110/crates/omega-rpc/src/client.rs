ï»¿// crates/omega-rpc/src/client.rs
//
...
//   the client reconnects it continues publishing to the same sender â€”
...
pub struct RpcClientConfig {
    ...
    pub rps_limit: u32,
    /// EIP-155 chain ID â€” used to stamp outbound signals.
    pub chain_id: u64,
}
...
impl OmegaRpcClient {
    ...
    pub async fn connect(config: RpcClientConfig) -> anyhow::Result<Self> {
        ...
        let _provider = ProviderBuilder::new().connect(WsConnect::new(ws)).await
    ...
    /// Uses backoff: 1s â†’ 2s â†’ 4s â†’ 8s â†’ â€¦ â†’ 30s cap.
    ...
    pub async fn run_block_subscription(&self) {
        ...
        loop {
            match self.run_block_subscription_once().await {
                Ok(()) => {
                    // Stream ended cleanly â€” reconnect
    ...
    async fn run_block_subscription_once(&self) -> anyhow::Result<()> {
        self.rate_limiter.wait_until_allowed(RpcRequestKind::Subscribe).await;

        let ws       = WsConnect::new(&self.config.ws_url);
        let provider = ProviderBuilder::new().connect(WsConnect::new(config.ws_url.clone())).await
        ...
        while let Some(block) = stream.next().await {
            ...
            // Ignore send error â€” no active receivers is fine during startup
    ...
    // â”€â”€ Rate-limited calls â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    /// not have a hard timeout â€” pass `wait_timeout` of `None` to wait
        ...
        Fut: std::future::Future<Output = anyhow::Result<T>>,
    {
        match wait_timeout {
            Some(timeout) => {
                tokio::time::timeout(timeout, self.rate_limiter.wait_until_allowed(RpcRequestKind::Read))
                    .await
                    .map_err(|_| anyhow::anyhow!("RPC read rate-limit timeout"))?;
            }
            None => {
                self.rate_limiter.wait_until_allowed(RpcRequestKind::Read).await;
            }
        }
        f().await
    }
    ...
    where
        ...
        Fut: std::future::Future<Output = anyhow::Result<T>>,
    {
        self.rate_limiter.wait_until_allowed(RpcRequestKind::Write).await;
        f().await
    }

    // â”€â”€ Fee oracle â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

    /// Fetch the current fee snapshot from the node.
    ///
    /// Reads the latest block's base fee and constructs a `FeeSnapshot`.
    /// The L1 data fee is set to 0 on non-Arbitrum chains (placeholder â€”
