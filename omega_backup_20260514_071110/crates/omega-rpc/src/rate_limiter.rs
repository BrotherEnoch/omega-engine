...
//
// ## Spec constraints (Â§22, hardware spec)
//
...
//   Write:      50 capacity,  50 refill/s  (max 1 write/cycle â€” conservative)
//   Subscribe:  20 capacity,  20 refill/s  (subscriptions are long-lived; low churn)
//
// ## Microtx constraint (Â§4)
//
//   Max 8 reads per Microtx blueprint.  At 400 reads/s with ~50Âµs per
...
//   would waste CPU.  The Tokio executor is not blocked â€” it is an
//   async sleep.
//
// ## Thread safety
//
//   `RpcRateLimiter` is `Clone` and `Send + Sync`.  The inner state
//   is wrapped in `Arc<Mutex<â€¦>>`.  The Mutex is async (Tokio) â€” it
...
pub enum RpcRequestKind {
    ...
    /// queries.  Up to 8 per Microtx blueprint (Â§4).
    Read,
    ...
    /// Long-lived connections â€” low churn.
    Subscribe,
}
...
///
/// `Clone` â€” all clones share the same underlying state via `Arc`.
...
impl RpcRateLimiter {
    ...
    /// than polling on a fixed 5ms interval â€” this reduces unnecessary
    ...
    pub async fn wait_until_allowed(&self, kind: RpcRequestKind) {
        loop {
            ...
            if wait_secs <= 0.0 {
                // Token is available â€” try to consume
...
mod tests {
    ...
    async fn depleted_read_bucket_throttles() {
        ...
        let rl = RpcRateLimiter::with_config(
            BucketConfig { capacity: 2, refill_per_second: 1 },
            BucketConfig::write_default(),
            BucketConfig::subscribe_default(),
        );
        assert!(rl.allow(RpcRequestKind::Read).await);
    ...
    async fn write_throttle_does_not_affect_reads() {
        let rl = RpcRateLimiter::with_config(
            BucketConfig::read_default(),
            BucketConfig { capacity: 1, refill_per_second: 1 },
            BucketConfig::subscribe_default(),
        );
    ...
    async fn snapshot_counts_throttled() {
        let rl = RpcRateLimiter::with_config(
            BucketConfig { capacity: 1, refill_per_second: 100 },
            BucketConfig::write_default(),
            BucketConfig::subscribe_default(),
        );
    ...
    async fn wait_until_allowed_returns_after_refill() {
        // 2-token bucket at 1000 rps â€” refills very fast
        let rl = RpcRateLimiter::with_config(
            BucketConfig { capacity: 2, refill_per_second: 1_000 },
            BucketConfig::write_default(),
            BucketConfig::subscribe_default(),
        );
        // Drain the bucket
