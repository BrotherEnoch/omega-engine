...
//
// ChaosTarget â€” injectable system state for chaos scenario execution.
//
// ## Purpose
//
...
// ## Thread safety
//
//   `ChaosTarget` is `Send + Sync` â€” scenarios may be run concurrently
...
/// and active faults are stored in a `Vec` â€” no `HashMap` key usage exists.
#[derive(Debug, Clone, PartialEq)]
pub enum FaultKind {
    /// Oracle feed is stale â€” `received_at` is more than `stale_secs` old.
    OracleStale { feed: String, stale_secs: u64 },
    /// Two oracle feeds diverge beyond the threshold.
    OracleDiverge { feed_a: String, feed_b: String, diverge_bps: u32 },
    /// Sequencer is not producing blocks.
    SequencerDown,
    /// Sequencer has just restarted after a gap â€” double-spend risk window.
    ...
    /// RPC token bucket is at zero tokens â€” all requests throttled.
    RpcRateExhausted,
}
...
impl std::fmt::Display for FaultKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::GasSpike { fee_gwei }
                => write!(f, "GasSpike({fee_gwei}gwei)"),
            Self::RelayTimeout
                => f.write_str("RelayTimeout"),
            Self::DagCycle { cycle_description }
                => write!(f, "DagCycle({cycle_description})"),
            Self::ZkProofDelay { latency_ms }
                => write!(f, "ZkProofDelay({latency_ms}ms)"),
            Self::LayerHalted { layer, reason }
                => write!(f, "LayerHalted({layer}: {reason})"),
            Self::RevmCacheStale { blocks_behind }
                => write!(f, "RevmCacheStale({blocks_behind}blk behind)"),
            Self::FlashloanDry { provider }
                => write!(f, "FlashloanDry({provider})"),
            Self::HighCompetition { win_pct }
                => write!(f, "HighCompetition({win_pct:.0}% competitor wins)"),
            Self::RpcRateExhausted
                => f.write_str("RpcRateExhausted"),
        }
    }
}
...
pub struct ChaosTarget {
    /// Health controllers for all 16 layers â€” real `LayerHealthImpl` instances.
...
impl ChaosTarget {
    ...
    pub fn new() -> Arc<Self> {
        ...
        let layer_ids = [
            ...
            Dag, Zk, HotPath, Strategy, Flashloan, Orchestrator,
            Relay, Vault, Observability, LossAttribution,
        ];
    ...
    // â”€â”€ Fault injection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    pub fn inject(&self, fault: FaultKind) {
        ...
        match &fault {
            ...
            FaultKind::SequencerDown | FaultKind::RelayTimeout => {
                use omega_core::LayerHealth;
                if let Some(ctrl) = self.health_layers.get(&LayerId::ExternalData) {
    ...
    /// Remove all active faults â€” used to test recovery.
    pub fn clear_faults(&self) {
        tracing::info!("CHAOS: all faults cleared â€” recovery phase");
    ...
    // â”€â”€ Observation recording â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Health state queries â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
impl Default for ChaosTarget {
    fn default() -> Self {
        Arc::try_unwrap(Self::new())
            .unwrap_or_else(|_| unreachable!("new() returns a unique Arc"))
    }
}