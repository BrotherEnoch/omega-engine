...
//
// Lifecycle:
//   1. `record(req)` â€” called when a ProofRequest is dequeued by a worker and
//      proof generation begins.
//   2. `complete(id)` â€” called when proof generation finishes (success or failure).
//      Removes the checkpoint entry.
//   3. `recover()` â€” called on startup; returns all incomplete entries so the
...
//   Disk writes are synchronous (no async IO contention on the hot path â€” only
...
// â”€â”€â”€ Checkpoint entry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
impl ProofCheckpointManager {
        ...
        if !dir.exists() {
            if let Err(e) = fs::create_dir_all(&dir) {
                tracing::warn!(
                    dir = %dir.display(),
                    error = %e,
                    "failed to create checkpoint dir â€” checkpoints disabled"
                );
            }
        }
        Arc::new(Self {
    ...
    pub fn record(
        ...
        if let Err(e) = self.write_to_disk(&entry) {
            tracing::warn!(
                request_id,
                error = %e,
                "checkpoint write failed â€” proof may be lost on restart"
            );
        } else {
    ...
    // â”€â”€ Recover â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    ///
    /// Returns a `Vec<ProofCheckpointEntry>` â€” caller submits each to the queue.
            ...
            match fs::read_to_string(&path) {
                    ...
                    match serde_json::from_str::<ProofCheckpointEntry>(&contents) {
                        ...
                        Err(e) => {
                            tracing::warn!(
                                path = %path.display(),
                                error = %e,
                                "malformed checkpoint file â€” skipping"
                            );
                        }
                    }
                }
                Err(e) => {
                    tracing::warn!(
                        path = %path.display(),
                        error = %e,
                        "failed to read checkpoint file â€” skipping"
                    );
                }
            }
        }
...
mod checkpoint_tests {
    ...
    fn temp_cfg() -> ZkConfig {
        ...
        cfg.checkpoint_dir = env::temp_dir()
            .join(format!("omega_zk_ckpt_{}", rand::random::<u32>()))
            .to_str()
            .unwrap()
            .to_string();
        cfg.max_checkpoints = 10;
        cfg
    }
    ...
    fn recover_returns_written_entries() {
        ...
        // Drop and recreate â€” simulates restart.
        drop(m);
    ...
    fn prune_keeps_max_checkpoints() {
        ...
        // After 5 records with max=3, in-memory count should be â‰¤ 3.
        assert!(m.in_flight_count() <= 3,
            "expected â‰¤3 in-flight, got {}", m.in_flight_count());
    }
