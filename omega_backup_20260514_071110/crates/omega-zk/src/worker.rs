...
//
// Design:
//   â€¢ N independent tokio tasks (one per configured worker_count, default = CPU count â‰¤ 8).
//   â€¢ Each worker loops: recv from crossbeam channel â†’ spawn_blocking(prove) â†’ send response.
//   â€¢ `spawn_blocking` offloads the CPU-bound Winterfell STARK prover to a dedicated
//     blocking thread pool (tokio's rayon-equivalent), keeping the async runtime free.
//   â€¢ SLA enforcement: tokio::time::timeout wraps each spawn_blocking call.
//     - Microtx: 1200 ms.
//     - Normal:  4000 ms.
//   â€¢ On timeout: the ProofRequest receives `Err(ZkError::ProofTimeout)`; the worker
//     continues to the next request without panicking.
//   â€¢ On prover panic: the JoinHandle error is caught; `WORKER_PANICS` metric incremented;
//     the worker restarts its loop (never crashes the pool).
//   â€¢ Worker pool shutdown: `ProofWorkerPool::shutdown()` closes the queue sender,
//     causing all `recv()` calls to return `Err(Disconnected)`, terminating workers cleanly.
//
// Parallelism:
//   Multiple workers drain the queue concurrently â€” max TPS = worker_count Ã— 1/proof_time.
//   For T1Software (~500ms per proof), 8 workers â†’ ~16 proofs/second throughput.
//
// Queue interaction:
...
// â”€â”€â”€ Worker loop â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    loop {
        ...
        let req = {
            ...
            match tokio::task::spawn_blocking(move || receiver.recv()).await {
                Ok(Ok(req))  => req,
                Ok(Err(_))   => {
                    // Channel disconnected â€” shutdown.
                    tracing::info!(worker_id, "proof worker channel closed, exiting");
                    break;
                }
                Err(join_err) => {
                    // spawn_blocking panicked â€” should never happen for a simple recv.
...
mod worker_tests {
    ...
    async fn worker_pool_generates_proof_end_to_end() {
        ...
        cfg.worker_count    = 2;
        // Relax SLA for CI (CI may be slow).
